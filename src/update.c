...
int hit_gain( CHAR_DATA *ch ) {
	...
	switch ( ch->position )
	{
	case POS_STUNNED:  gain += 10;
	case POS_SLEEPING: gain += get_curr_con(ch)/3 * 2.0;
		if(ch->on) gain *= 2;
		break;
	case POS_RESTING:  gain += get_curr_con(ch)/3 * 1.25;
		if(ch->on) gain *= 1.25;
		break;
	}

        if (ch->nation) gain *= ch->nation->metabolism;
	gain *= ch->speed / 100;
    ...
    for (part = ch->first_part;part;part = part->next) {
		...
		if (part->cond < 0) {
			...
			act(AT_BLOOD, "Blood clouds the water.",
...
int mana_gain( CHAR_DATA *ch )
{
	...
	switch ( ch->position )
	{
	case POS_SLEEPING: gain += get_curr_int(ch)/3 * 3.25; if(ch->on) gain += 5;	break;
	case POS_RESTING:  gain += get_curr_int(ch)/3 * 1.75; if(ch->on) gain += 2;	break;
	default:
...
int move_gain( CHAR_DATA *ch )
{
	...
	{
	case POS_STUNNED:
	case POS_SLEEPING: gain += get_curr_dex(ch)/3 * 4.5;
		if(ch->on) gain *= 3;
		...
		gain *= ch->speed / 100;
                ...
                if (IS_AFFECTED(ch, AFF_SLOW)) gain /= 2;
		break;
	case POS_RESTING:  gain += get_curr_dex(ch)/3 * 2.5;
		if(ch->on) gain *= 2;
                gain *= ch->nation->metabolism;
		gain *= ch->speed / 100;
                if (IS_AFFECTED(ch, AFF_HASTE)) gain *= 2;
...
bool climate_affect( CHAR_DATA *ch, int type ) {
	...
	if (type == MAG_FIRE) {
                if (IS_SET(ch->immune, RIS_FIRE)) return FALSE;
                if (IS_SET(ch->resistant, RIS_FIRE)) return FALSE;
	...
	if (type == MAG_COLD) {
                if (IS_SET(ch->immune, RIS_COLD)) return FALSE;
                if (IS_SET(ch->resistant, RIS_COLD)) return FALSE;
...
void mobile_update( void )
{
    ...
    for ( ch = last_char; ch; ch = gch_prev )
    {
		...
		if (xIS_SET(ch->act, ACT_SENTINEL) 
			...
			act(AT_PLAIN, "$n disappears in a swirl of mist.",
				ch, NULL, NULL, TO_ROOM);
			char_from_room(ch);
		...
		if (ch->in_room && ch->in_room->area->weather->precip > 10
			&& !IS_SET(ch->in_room->room_flags, ROOM_INDOORS)) {
			OBJ_DATA *pobj;

			for (pobj=ch->in_room->first_content; pobj; pobj=pobj->next_content)
				if (number_range(0, 4) < 1 && xIS_SET(pobj->extra_flags, ITEM_BURIED)) {
		...
		if (ch->last_carrying && !xIS_SET(ch->act, ACT_PACIFIST)
		&&  IS_SET(ch->xflags, PART_HANDS)) {
		...
		if ( ch->in_room->first_content && IS_SET(ch->xflags, PART_HANDS))
		{
			...
			if ((xIS_SET(ch->act, ACT_SCAVENGER) || number_percent() < 35)
...
void char_regen( void ) {
	...
	if (!IS_NPC(ch)) {
		...
		else if (ch->mana > ch->max_mana ) { /* mana burn */
...
void char_update( void ) {   
	...
	if ( !char_died(ch) )
	{
	     ...
	     */
	    if (IS_AFFECTED(ch, AFF_ROTTING)
	    && !IS_SET(ch->immune, RIS_DRAIN)) {
...
void scatter_arti(OBJ_DATA *obj) {
    ...
    for ( ;; )
    {
        ...
        if (obj->carried_by && obj->carried_by->name) {
                act(AT_MAGIC, "$p vanishes in a puff of smoke.",
		obj->carried_by, NULL, NULL, TO_CHAR);
                obj_from_char(obj);
        }
        if (obj->in_obj)
                obj_from_obj(obj);
...
{   
	...
	if (!obj->carried_by && !obj->in_room && !obj->in_obj) {
	  if (IS_OBJ_STAT(obj, ITEM_ARTIFACT)) scatter_arti(obj);
	...
	if ( obj->carried_by )
	{
	    ...
	    if (IS_OBJ_STAT(obj, ITEM_EXPLOSIVE)) {
		player_echo( obj->carried_by, AT_ACTION,
		"An explosion echoes throughout the area.", ECHOTAR_AREA);
    ...
    for ( ch = first_char; ch; ch = ch_next )
    {
		...
		/* Both NPC and PC (Why should only PC's drown?) */
			...
			|| ( ch->in_room && ch->in_room->sector_type == SECT_OCEANFLOOR ) )
			{
				...
				&&   !IS_AFFECTED( ch, AFF_AQUATIC )
				&&   !(ch->in_obj && IS_SET(ch->in_obj->value[2], VEHICLE_AIRTIGHT))) {
					int dam;
...
void drunk_randoms( CHAR_DATA *ch )
{
    ...
    if ( drunk > (20+(get_curr_con(ch)/5))
    &&   number_percent() < ( drunk / 30 ) ) {
	uncon(ch);
    }
...
void adjust_vectors(WEATHER_DATA *weather)
{
	...
	dM += climate_factor *
		(((weather->climate_mana - 2)*weath_unit) -
		(weather->wind))/weath_unit;
			...
			neigh = neigh->next)
	{
		...
		dM +=(neigh->address->weather->mana -
			  weather->mana) / neigh_factor;
	}
	...
	weather->mana_vector = URANGE(-max_vector,
		weather->mana_vector, max_vector);
...
if (IS_SET(sysdata.quest, QUEST_TEMPEST)) {
		...
		pArea->weather->mana = URANGE(-limit,
			pArea->weather->mana, limit);
			...
			if(precip - dP > 0)
			{
				...
				if (moon_visible(tarea, MOON_BLOOD)) {
					char *echo_strings[4] =
					{
						...
						"The blood rain tapers off.\n\r",
						"The blood rain stops falling.\n\r",
...
if (IS_SET(sysdata.quest, QUEST_ETERNAL_NIGHT)) {
        ...
        char *echo_strings[4] =
        {
             ...
             "Eternal night has the realms in its grasp.\n\r",
             "Darkness covers the entire sky.\n\r",
             "The starless skies lie oppressive above you.\n\r"
        };
	...
	if(time_info.month >= 12)
	{
		 ...
		 */
                for(d = first_descriptor; d; d = d->next)
                {
                        ...
                        {
                                ch_printf(d->character,
                                "^x&RFireworks burst in the sky as people celebrate the coming of the Year %d.\n\r",
                                time_info.year);
                        }
		}
	}
...
void hunger_thirst_update() {
	...
	for (d=first_descriptor; d; d=d->next) {
		...
		switch( ch->position )
		{
			...
			case POS_RESTING:   better_mental_state( ch, 5 ); break;
			case POS_SITTING:
