...
void do_sell( CHAR_DATA *ch, char *argument ) {
   ...
   ch_printf( ch, "and &O%dcopper&w coins.\n\r", gp);
	...
	for ( obj = ch->first_carrying; obj; obj = obj_next )
	{
		...
		if (IS_OBJ_STAT(obj, ITEM_DREAMWORLD)) continue;

		if ((cost = get_cost(ch, ch->in_room->pShop, obj, FALSE)) <= 0) continue;

		if ( ch->in_room->pShop->type != -1
		&&   ch->in_room->pShop->type != obj->item_type ) continue;

		if ( ch->in_room->pShop->flag != -1
		&&  !xIS_SET(obj->extra_flags, ch->in_room->pShop->flag ) ) continue;
...
void do_shops( CHAR_DATA *ch, char *argument ) {
	...
	pager_printf( ch, "Room: %5d Salary: %3d Sell: %3d Type: %d Flag: %d Owner: %s\n\r",
		shop->room,	   shop->profit_buy, shop->profit_sell,
