...
/*  gettimeofday( &boot_time, NULL);   okay, so it's kludgy, sue me :) */
	...
	else if ( ( port = atoi( argv[1] ) ) <= 1024 )
	{
       ...
       if (argv[2] && argv[2][0])
       {
          ...
          conjava = atoi(argv[6]);
#ifdef I3
	   I3_socket = atoi( argv[7] );
#endif
#ifdef IMC
          imcsocket = atoi( argv[8] );
#endif
       }
       else
          fCopyOver = FALSE;
...
void nanny( DESCRIPTOR_DATA *d, char *argument ) {
	...
	ch->position = POS_STANDING;
	if (d->connected == CON_GET_WANT_RIPANSI) d->connected = CON_PLAYING;
	...
	act( AT_ACTION, "$n has awakened again into the world.", 
	...
	sprintf(buf, "%s has entered the game in %s (%d).",
    ...
    if ( ch->pcdata->pet )
    {
           ...
           act( AT_ACTION, "$N returns with you to the planes.",
  ...
  while ( str && *str != '\0' )
  {
	...
	case '$': 
      ...
      case 'p': i = (!to || can_see_obj(to, obj1)
		  ? aoran(myobj(obj1)) : "something"); break;
      case 'P': i = (!to || can_see_obj(to, obj2)
		  ? aoran(myobj(obj2)) : "something"); break;
      case 'd':
...
void act( sh_int AType, const char *format, CHAR_DATA *ch, const void *arg1, const void *arg2, int type )
{
	...
	||   !IS_AWAKE(to) || 
	(!CAN_SEE_PLANE(to, ch)
	&& (to->curr_talent[TAL_SEEKING] < 100) ) )
	    continue;
...
void display_prompt( DESCRIPTOR_DATA *d )
{
  ...
  else if (IS_NPC (ch) || (!IS_FIGHTING(ch) && (!ch->pcdata->prompt
                || !*ch->pcdata->prompt) ) )
...
/*  make_color_sequence(NULL, NULL, NULL);*/
  for ( ; *prompt; prompt++ )
  {
	...
	*pbuf = '\0';
	...
	else if ( time_info.hour < 12 ) strcpy( pbuf, "morning" );
	else if ( time_info.hour < 13 ) strcpy( pbuf, "noon" );
	else if ( time_info.hour < 16 ) strcpy( pbuf, "afternoon" );
        else if ( time_info.hour < 20 ) strcpy( pbuf, "evening" );
	...
	break;
      case 'r':
	  sprintf( pbuf, IS_SET(ch->mood, MOOD_READY) ?
		"ready" : "relaxed" );
	break;
      case 'F':
...
void do_crashguard ()
{
  ...
  char buf [100], buf2[100], buf3[100], buf4[100], buf5[100], buf6[100], buf7[100];
...
#ifdef IMC
   if( this_imcmud )
      snprintf( buf7, 100, "%d", this_imcmud->desc );
   else
      strncpy( buf7, "-1", 100 );
#else
   strncpy( buf7, "-1", 100 );
        ...
        execl (EXE_FILE, "rmexe", buf, "copyover", buf2, buf3,
          buf4, buf5, buf6, buf7, (char *) NULL);
...
void sig_handler(int sig)
{
  ...
  /* Ignor SIGSEGV and SIGBUS by Josh - Signal Blocking by Josh */
	...
	for ( ch = first_char; ch; ch = ch->next ) {
		if (IS_NPC(ch))
			continue;
		sprintf( buf, "PC: %s room: %d", ch->name,
			ch->in_room ? ch->in_room->vnum : -1 );
		log_string( buf );
	}

	if (sysdata.crashguard == FALSE) {/* in case I -want- a core dump -- Scion */
		switch(sig) {
		case SIGBUS:
			bug("Signal SIGBUS.",0);
		case SIGTERM:
