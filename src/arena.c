...
void ko_message(ROOM_INDEX_DATA *room, ARENA_MEMBER *victim, ARENA_MEMBER *attacker ) {
	...
	switch (number_range(0,10)) {
		...
		victim->name, attacker->name, attacker->name, attacker->name);
		break;
	    case 7:
...
int get_odds(ARENA_MEMBER *calc) {
	...
	for (member=calc->arena->first_member;member;member=member->next) {
		if (IS_SET(member->flags, GLAD_WAIT)) continue;
		diff += member->played;
		if (member->team != 0 &&
		member->team == calc->team) team += member->played;
		else opp++;
		count++;
	}
...
void arena_update( void ) {
	...
	while(arena) {
		...
		if (team != 0) arena->phase = 0;
		}
		if (count <= 1) arena->phase = 0;
		else {
		i = 0;
		...
		for (member=arena->first_member;member;member=member->next) {
			...
			for (winner=arena->first_member;winner;winner=winner->next) {
				if (winner->hp <= 0) continue;
				if (IS_SET(winner->flags, GLAD_WAIT)) continue;
				if (winner == member) continue;
				if (IS_SET(arena->flags, ARENA_TEAM)
				&&   winner->team != 0
				&&   member->team == winner->team) continue;
				count++;
			}
			num = number_range(1,count);
			if (IS_SET(arena->flags, ARENA_DEBUG))
		...
		/* Announce the winner */
			...
			for (member=arena->first_member;member;member=member = winner) {
				...
				switch (number_range(1,5)) {
					case 1: member->str--; break;
					case 2: member->con--; break;
					case 3: member->dex--; break;
					case 4: member->spd--; break;
					case 5: member->end--; break;
				 }
			if (member->str <= 0
			||  member->con <= 0
			||  member->dex <= 0
...
void do_arena(CHAR_DATA *ch, char *argument) {
	...
	if (!str_cmp(arg1, "list")) {
		...
		for (member=arena->first_member;member;member=member->next) {
			...
			if (member->hp <= 0) ch_printf(ch, "&r");
			else if (IS_SET(member->flags, GLAD_WAIT)) ch_printf(ch, "&b");
			else ch_printf(ch, "&Y");
			...
			ch_printf(ch, "  &gSTR: &G%d  &gCON: &G%d  &gDEX: &G%d  &gSPD: &G%d  ",
				...
				member->dex, member->spd);
			if (IS_SET(member->flags, GLAD_RANDOM)) ch_printf(ch, "&r");
			else ch_printf(ch, "&g");
...
ch_printf(ch, " &O%dc!\n\r", gp);
	...
	if (!strcmp(arg1, "find")) {
		...
		while (count>0) {
			...
			i, new_member->name, new_member->score, new_member->total);
			last = score; 
			i++;
			count--;
		}
...
ch_printf(ch, " &O%dc on %s.\n\r", gp, member->name);
		...
		for (member=arena->first_member;member;member=member->next) {
			for (bet=member->first_bet;bet;bet=bet->next) {
				if (str_cmp(bet->better, ch->name)) continue;
				if (bet->winnings == 0) continue;
...
ch_printf(ch, " &O%dc.\n\r", gp);
		...
		for (member=arena->first_member;member;member=member->next) {
			if (!str_cmp(member->name, arg3)) {
				...
				if (IS_SET(member->flags, GLAD_TOGGLE)) {
					...
					member->name, IS_SET(member->flags, GLAD_WAIT) ? "leave" : "enter");
				}
				return;
			}
		}
		...
		for (member=arena->first_member;member;member=member->next) {
			if (!str_cmp(member->name, arg3)) {
				...
				if (!str_cmp(arg4, "str")) member->str++;
				else if (!str_cmp(arg4, "con")) member->con++;
				else if (!str_cmp(arg4, "dex")) member->dex++;
				else if (!str_cmp(arg4, "spd")) member->spd++;
				else {
	...
	if (!strcmp(arg1, "topten")) {
		...
		for (member=arena->first_member;member;member=member->next) {
			if (!str_cmp(member->name, arg3)) {
				...
				count=i=0;
				for (new_arena=first_arena;new_arena;new_arena=new_arena->next) {
					if (IS_SET(new_arena->flags, ARENA_HALT)) continue;
					if (IS_SET(new_arena->flags, ARENA_TEAM)) continue;
					count++;
				}
