	...
	if (!argument || argument[0] == '\0') {
		for (talent = first_deity; talent; talent = talent->next) {
			sprintf(buf, "&Y%-20s: %-3d  &CCost per point: %d\r\n", 
				talent->name, 
				(ch->talent[talent->index] > 0 ? ch->talent[talent->index] : 0),
				talent->cost);
			send_to_char(buf, ch);
		}
...
void use_magic(CHAR_DATA *ch, int tal, int mana) {
	...
	if (number_range(1,5) == 1) learn_noncombat(ch, SK_CHANNEL);
	if (mana < 0) mana = 0;
...
CHAR_DATA *find_target(CHAR_DATA *ch, char *argument, bool offense) {
	...
	CHAR_DATA *victim = NULL;
	    ...
	    if (!offense)
                victim = ch;
	    else if (ch->last_hit && ch->last_hit->in_room ?
	    ch->last_hit->in_room == ch->in_room : FALSE)
		victim = ch->last_hit;
        } else {
	...
	if (!victim || victim == NULL) {
	    if (!offense && ch->curr_talent[TAL_MOTION] >= 100
...
void do_status(CHAR_DATA *ch, char *argument) {
	...
	if (ch->mental_state == 100)
		...
		ch_printf(ch, "You are extremely drousy.\n\r");
	else if (ch->mental_state < 0)
...
void do_heal(CHAR_DATA *ch, char *argument) {
	...
	if (IS_AFFECTED(victim, AFF_UNDEAD)) {
	        magic_damage(victim, ch, TALENT(ch, TAL_HEALING)*3
		+ TALENT(ch, TAL_DEATH),
	        MAG_DRAIN, TAL_HEALING, FALSE);
		return;
	}
	...
	amt = number_range(TALENT(ch, TAL_HEALING),
		TALENT(ch, TAL_HEALING)*5);
	/* take mana first so the bonus doesn't cost more */
	use_magic(ch, TAL_HEALING, amt);
	if (IS_AFFECTED(ch, AFF_UNHOLY) && amt != 0) amt /= 2;
	if (IS_AFFECTED(ch, AFF_HOLY)) amt *= 2;
	if (victim == ch) amt *= 2;
	if (!IS_FIGHTING(ch)) amt *= 2;
	victim->hit += amt;
	...
	if (victim != ch && ch->curr_talent[TAL_DEATH] > 0) {
		amt = number_range(TALENT(ch, TAL_DEATH),
		TALENT(ch, TAL_DEATH)*5);
...
void do_burn(CHAR_DATA *ch, char *argument) {
	...
	act(AT_FIRE, "Flames fly from your $t!",
		ch, hands(ch), NULL, TO_CHAR);
	act(AT_FIRE, "Flames fly from $n's $t!",
		ch, hands(ch), NULL, TO_ROOM);
	...
	IS_UNDERWATER(ch) ?
	TALENT(ch, TAL_FIRE)/2 : TALENT(ch, TAL_FIRE),
	MAG_FIRE, TAL_FIRE, FALSE);

        if (ch->curr_talent[TAL_FIRE] >= 50 &&
        !IS_SET(victim->immune, RIS_FIRE) &&
	!(IS_NPC(ch) && xIS_SET(ch->act, ACT_PACIFIST))) {
           af.type      = gsn_sticky_flame;
           af.duration  = 20;
...
void do_poisoncloud(CHAR_DATA *ch, char *argument) {
	...
	if (ch->curr_talent[TAL_DEATH]
	+   ch->curr_talent[TAL_WIND] < 200) {
                huh(ch);
                return;
        }

        act(AT_DGREEN, "A cloud of poisonous gas erupts from your $t!",
                ch, hands(ch), NULL, TO_CHAR);
        act(AT_DGREEN, "A cloud if poisonous gas erupts from $n's $t!",
                ch, hands(ch), NULL, TO_ROOM);

    for ( victim=ch->in_room->first_person; victim; victim=next_vict ) {
        next_vict=victim->next_in_room;

        magic_damage(victim, ch, (TALENT(ch, TAL_DEATH)
		+ TALENT(ch, TAL_WIND)),
                MAG_POISON, TAL_DEATH, FALSE);
        if (char_died(ch))
                return;
...
void do_quake(CHAR_DATA *ch, char *argument) {
	...
	act(AT_HUNGRY, "The earth trembles at your command!",
		ch, NULL, NULL, TO_CHAR);
	...
	&& !IS_SET(ch->in_room->room_flags, ROOM_INDOORS))
        ...
        magic_damage(victim, ch, TALENT(ch, TAL_EARTH),
		MAG_BLUNT, TAL_EARTH, FALSE);
	if (char_died(ch))
		return;
    }
}
...
void do_telekinetics(CHAR_DATA *ch, char *argument) {
	...
	if (!IS_NPC(ch) && (ch->curr_talent[TAL_MOTION] < 20
	||  ch->curr_talent[TAL_MIND] < 10)) {
		huh(ch);
		return;
	}
	...
	n++;
        ...
        magic_damage(victim, ch,
        (ch->curr_talent[TAL_MIND] + ch->curr_talent[TAL_MOTION])/2,
        MAG_PSIONIC, TAL_MOTION, FALSE);
        ...
        learn_talent(ch, TAL_MIND);
        magic_damage(victim, ch,
        TALENT(ch, TAL_MIND) + TALENT(ch, TAL_MOTION),
        MAG_PSIONIC, TAL_MOTION, FALSE);
}
...
void do_sandstorm(CHAR_DATA *ch, char *argument) {
	AFFECT_DATA af;
        ...
        act(AT_ORANGE, "Blasting sand swirls from your $t!",
                ch, hands(ch), NULL, TO_CHAR);
        act(AT_ORANGE, "Blasting sand swirls from $n's $t!",
                ch, hands(ch), NULL, TO_ROOM);
    ...
    for ( victim = ch->in_room->first_person; victim; victim = next_vict )
    {
        ...
        magic_damage(victim, ch,
        (TALENT(ch, TAL_WIND) + TALENT(ch, TAL_EARTH))/2,
        ...
        learn_talent(ch, TAL_EARTH);
        magic_damage(victim, ch,
        TALENT(ch, TAL_EARTH) + TALENT(ch, TAL_WIND),
        MAG_WIND, TAL_WIND, FALSE);
...
void do_magma(CHAR_DATA *ch, char *argument) {
	...
	||  ch->curr_talent[TAL_FIRE] < 1) && !IS_NPC(ch)) {
                huh(ch);
                return;
        }

        act(AT_FIRE, "Molten lava pours from your $t!",
                ch, hands(ch), NULL, TO_CHAR);
        act(AT_FIRE, "Molten lava pours from $n's $t!",
                ch, hands(ch), NULL, TO_ROOM);
	...
	n++;
        ...
        magic_damage(victim, ch,
        (TALENT(ch, TAL_FIRE) + TALENT(ch, TAL_EARTH))/2,
        MAG_FIRE, TAL_EARTH, FALSE);
	...
	learn_talent(ch, TAL_EARTH);
        magic_damage(victim, ch,
	TALENT(ch, TAL_EARTH) + TALENT(ch, TAL_FIRE),
        MAG_FIRE, TAL_FIRE, FALSE);
}
...
void do_flare(CHAR_DATA *ch, char *argument) {
	...
	int n=0;
        ...
        act(AT_YELLOW, "Searing light radiates from your $t!",
                ch, hands(ch), NULL, TO_CHAR);
        act(AT_YELLOW, "Searing light radiates from $n's $t!",
                ch, hands(ch), NULL, TO_ROOM);
	...
	n++;
        ...
        magic_damage(victim, ch,
        (TALENT(ch, TAL_CATALYSM) + TALENT(ch, TAL_ILLUSION))/2,
        MAG_LIGHT, TAL_ILLUSION, FALSE);
        ...
        learn_talent(ch, TAL_CATALYSM);
        magic_damage(victim, ch,
        TALENT(ch, TAL_CATALYSM) + TALENT(ch, TAL_ILLUSION),
        MAG_LIGHT, TAL_ILLUSION, FALSE);
}
...
void do_antimatter(CHAR_DATA *ch, char *argument) {
        ...
        act(AT_DGREY, "Crackling black antimatter streams from your $t!",
                ch, hands(ch), NULL, TO_CHAR);
        act(AT_DGREY, "Crackling black antimatter streams from $n's $t!",
                ch, hands(ch), NULL, TO_ROOM);

        victim = find_target(ch, argument, TRUE);
        if (!victim) return;

        magic_damage(victim, ch, TALENT(ch, TAL_VOID),
        MAG_ANTIMATTER, TAL_VOID, FALSE);
}
...
void do_chainlightning(CHAR_DATA *ch, char *argument) {
	...
	CHAR_DATA *next_vict = NULL;
        ...
        act(AT_YELLOW, "Chain lightning streaks from your $t!",
                ch, hands(ch), NULL, TO_CHAR);
        act(AT_YELLOW, "Chain lightning streaks from $n's $t!",
                ch, hands(ch), NULL, TO_ROOM);

	zaps = number_range(1, TALENT(ch, TAL_LIGHTNING)/25);
	if (ch->in_room->curr_water > 60) zaps *= 2;
	use_magic(ch, TAL_LIGHTNING, zaps*100);
	while (zaps--) {
		victim = next_vict;
		if (!next_vict) victim = ch->in_room->first_person;
		next_vict = victim->next_in_room;
	        magic_damage(victim, ch, 100,
	        MAG_ELECTRICITY, TAL_LIGHTNING, FALSE);
	}
}
...
void do_electrocute(CHAR_DATA *ch, char *argument) {
        ...
        if (ch->curr_talent[TAL_LIGHTNING]
        +   ch->curr_talent[TAL_WIND] < 200 && !IS_NPC(ch)) {
                huh(ch);
                return;
        }

        if (IS_SET(ch->in_room->room_flags, ROOM_INDOORS)
        ||  ch->in_room->area->weather->precip < 100
        ||  ch->in_room->area->weather->temp < 100) {
                send_to_char("The weather isn't stormy enough here to call lightning.\n\r", ch);
                return;
        }
        act(AT_YELLOW, "A dozen bolts of electricity merge in the sky and shoot down a powerful bolt!",
                ch, NULL, NULL, TO_CHAR);
        act(AT_YELLOW, "A dozen bolts of electricity merge in the sky and shoot down a powerful bolt!",
                ch, NULL, NULL, TO_ROOM);
        victim = find_target(ch, argument, TRUE);
...
void do_flamingrain(CHAR_DATA *ch, char *argument) {
	...
	CHAR_DATA *next_vict;
        ...
        act(AT_FIRE, "The sky turns red as fire rains from the heavens.",
                ch, NULL, NULL, TO_CHAR);
        act(AT_FIRE, "The sky turns red as fire rains from the heavens.",
		ch, NULL, NULL, TO_ROOM);
...
void do_manastorm(CHAR_DATA *ch, char *argument) {
	...
	CHAR_DATA *next_vict;
        ...
        if (ch->curr_talent[TAL_CATALYSM]
        +   ch->curr_talent[TAL_WIND] < 200 && !IS_NPC(ch)) {
                huh(ch);
                return;
        }
        ...
        if (IS_SET(ch->in_room->room_flags, ROOM_INDOORS)) {
                send_to_char("You can't call a manastorm indoors.\n\r", ch);
                return;
        }

        act(AT_PURPLE, "You make the energy rage in a manastorm!",
                ch, NULL, NULL, TO_CHAR);
        act(AT_PURPLE, "The energy in the air rages in a manastorm!",
                ch, NULL, NULL, TO_ROOM);
...
void do_meteor(CHAR_DATA *ch, char *argument) {
        ...
        if (ch->curr_talent[TAL_EARTH]
        +   ch->curr_talent[TAL_VOID] < 200 && !IS_NPC(ch)) {
                huh(ch);
                return;
        }
	if (IS_SET(ch->in_room->room_flags, ROOM_INDOORS)) {
        ...
        act(AT_DGREY, "A meteor streaks from the sky!",
                ch, NULL, NULL, TO_CHAR);
        act(AT_DGREY, "A meteor streaks from the sky!",
                ch, NULL, NULL, TO_ROOM);
        victim = find_target(ch, argument, TRUE);
        ...
        magic_damage(victim, ch,
        (TALENT(ch, TAL_EARTH) + TALENT(ch, TAL_VOID))*2,
        MAG_BLUNT, TAL_EARTH, FALSE);
}
...
void do_sonics(CHAR_DATA *ch, char *argument) {
	...
	if (ch->in_room->curr_water >= 100) {
        ...
        act(AT_YELLOW, "You make the air vibrate loudly!",
                ch, NULL, NULL, TO_CHAR);
        act(AT_YELLOW, "The very air seems to hum loudly!",
                ch, NULL, NULL, TO_ROOM);
	...
	learn_talent(ch, TAL_WIND);
        magic_damage(victim, ch, TALENT(ch, TAL_SPEECH) +
        TALENT(ch, TAL_MOTION) + TALENT(ch, TAL_WIND),
        MAG_SOUND, TAL_SPEECH, FALSE);
    }
}
...
void do_steam(CHAR_DATA *ch, char *argument) {
        ...
        if (ch->curr_talent[TAL_WATER]
        +   ch->curr_talent[TAL_FIRE] < 200 && !IS_NPC(ch)) {
                huh(ch);
                return;
        }
        act(AT_BLUE, "A burst of steam blasts from your $t!",
                ch, hands(ch), NULL, TO_CHAR);
        act(AT_BLUE, "A burst of steam blasts from $n's $t!",
                ch, hands(ch), NULL, TO_ROOM);
        victim = find_target(ch, argument, TRUE);
        if (!victim) return;
	...
	(IS_UNDERWATER(ch) ? 2 : 1),
        MAG_FIRE, TAL_WATER, FALSE);
}
...
void do_blizzard(CHAR_DATA *ch, char *argument) {
	...
	CHAR_DATA *next_vict;
        ...
        if (IS_SET(ch->in_room->room_flags, ROOM_INDOORS)
        ||  ch->in_room->area->weather->precip < 50
	||  ch->in_room->area->weather->temp > -100) {
                send_to_char("The weather isn't right here to call a blizzard.\n\r", ch);
                return;
        }
        act(AT_FROST, "A swirling blizzard blasts from your $t!",
                ch, hands(ch), NULL, TO_CHAR);
        act(AT_FROST, "A swirling blizzard blasts from $n's $t!",
                ch, hands(ch), NULL, TO_ROOM);
	...
	learn_talent(ch, TAL_WIND);
	magic_damage(victim, ch,
        (TALENT(ch, TAL_WIND) + TALENT(ch, TAL_FROST))*2,
        MAG_COLD, TAL_FROST, FALSE);
    }
}
...
void do_sunstroke(CHAR_DATA *ch, char *argument) {
	...
	||  get_light_room(ch->in_room) < 0) {
		send_to_char("There isn't enough sunlight here.\n\r", ch);
		return;
	}

        act(AT_YELLOW, "Your $t beam a ray of searing sunlight!",
                ch, hands(ch), NULL, TO_CHAR);
        act(AT_YELLOW, "$n's $t beam a ray of searing sunlight!",
                ch, hands(ch), NULL, TO_ROOM);
        victim = find_target(ch, argument, TRUE);
        if (!victim) return;
	...
	learn_talent(ch, TAL_FIRE);
	magic_damage(victim, ch,
        (TALENT(ch, TAL_ILLUSION) + TALENT(ch, TAL_FIRE))*2,
        MAG_LIGHT, TAL_ILLUSION, FALSE);
}
...
void do_soulfire(CHAR_DATA *ch, char *argument) {
        ...
        act(AT_DGREY, "Black fire streaks from your $t!",
                ch, hands(ch), NULL, TO_CHAR);
        act(AT_DGREY, "Black fire streaks from $n's $t!",
                ch, hands(ch), NULL, TO_ROOM);
        victim = find_target(ch, argument, TRUE);
        if (!victim) return;

        if (!IS_AFFECTED(victim, AFF_UNHOLY)
	&&  !IS_AFFECTED(victim, AFF_UNDEAD)
        &&  !IS_AFFECTED(victim, AFF_NONLIVING)) {
		learn_talent(ch, TAL_FIRE);
	        magic_damage(victim, ch,
	        (TALENT(ch, TAL_DEATH) + TALENT(ch, TAL_FIRE))*1.5,
	        MAG_UNHOLY, TAL_DEATH, FALSE);
        } else {
...
void do_unholyblast(CHAR_DATA *ch, char *argument) {
        ...
        act(AT_DGREY, "Unholy energy blasts from your $t!",
                ch, hands(ch), NULL, TO_CHAR);
        act(AT_DGREY, "Unholy energy blasts from $n's $t!",
                ch, hands(ch), NULL, TO_ROOM);

        victim = find_target(ch, argument, TRUE);
        if (!victim) return;

        if (IS_AFFECTED(victim, AFF_HOLY)
        ||  IS_AFFECTED(victim, AFF_ANGEL)) {
		learn_talent(ch, TAL_SECURITY);
	        magic_damage(victim, ch,
	        (TALENT(ch, TAL_DEATH) + TALENT(ch, TAL_SECURITY))*2,
	        MAG_UNHOLY, TAL_DEATH, FALSE);
        } else {
                act(AT_DGREY, "$N is unaffected.", ch, NULL, victim, TO_CHAR);
        }
...
void do_sworddance(CHAR_DATA *ch, char *argument) {
	...
	if (!ch->main_hand || ch->main_hand->value[5] != SK_SWORD) {
        ...
        act(AT_WHITE, "You dance with the sword, and a dozen other swords dance with you.",
                ch, NULL, NULL, TO_CHAR);
        act(AT_WHITE, "$n dances, a dozen whirling swords slashing around $n!",
                ch, NULL, NULL, TO_ROOM);
	...
	lose_ep(ch, 100);
        ...
        magic_damage(victim, ch,
        ch->main_hand->value[2] + ch->pcdata->weapon[SK_SWORD]
	+ ch->curr_talent[TAL_CATALYSM],
        MAG_SLASH, TAL_CATALYSM, FALSE);
...
void do_pyrokinetics(CHAR_DATA *ch, char *argument) {
        ...
        if (ch->curr_talent[TAL_MIND]
        +   ch->curr_talent[TAL_FIRE] < 200 && !IS_NPC(ch)) {
                huh(ch);
                return;
        }

        act(AT_BLUE, "Blazing blue psionic flames streak from your $t!",
                ch, hands(ch), NULL, TO_CHAR);
        act(AT_BLUE, "Blazing blue psionic flames streak from $n's $t!",
                ch, hands(ch), NULL, TO_ROOM);
        ...
        if (IS_AFFECTED(victim, AFF_CONSTRUCT)
        ||  IS_AFFECTED(victim, AFF_UNDEAD)) {
                magic_damage(victim, ch, TALENT(ch, TAL_FIRE),
                MAG_FIRE, TAL_FIRE, FALSE);
        } else {
		learn_talent(ch, TAL_FIRE);
                magic_damage(victim, ch,
                (TALENT(ch, TAL_MIND) + TALENT(ch, TAL_FIRE))*2,
                MAG_PSIONIC, TAL_MIND, FALSE);
		learn_talent(ch, TAL_FIRE);
        }
}
...
void do_geyser(CHAR_DATA *ch, char *argument) {
        ...
        if (ch->curr_talent[TAL_FIRE] + ch->curr_talent[TAL_WATER]
        +   ch->curr_talent[TAL_EARTH] < 100 && !IS_NPC(ch)) {
                huh(ch);
                return;
        }
	...
	if (IS_SET(ch->in_room->room_flags, ROOM_NOFLOOR)) {
        ...
        act(AT_BLUE, "A geyser erupts from the ground!",
                ch, NULL, NULL, TO_CHAR);
        act(AT_BLUE, "A geyser erupts from the ground!",
                ch, hands(ch), NULL, TO_ROOM);
	...
	learn_talent(ch, TAL_EARTH);
        magic_damage(victim, ch, TALENT(ch, TAL_EARTH) +
        TALENT(ch, TAL_WATER) + TALENT(ch, TAL_FIRE),
...
void do_brainshock(CHAR_DATA *ch, char *argument) {
        ...
        if (ch->curr_talent[TAL_MIND]
        +   ch->curr_talent[TAL_LIGHTNING] < 200 && !IS_NPC(ch)) {
                huh(ch);
                return;
        }

        act(AT_YELLOW, "A surge of psionic lightning bursts from your $t!",
                ch, hands(ch), NULL, TO_CHAR);
        act(AT_YELLOW, "A surge of psionic lightning bursts from $n's $t!",
                ch, hands(ch), NULL, TO_ROOM);
	...
	if (IS_AFFECTED(victim, AFF_CONSTRUCT)
	||  IS_AFFECTED(victim, AFF_UNDEAD)) {
		magic_damage(victim, ch, TALENT(ch, TAL_LIGHTNING),
		MAG_ELECTRICITY, TAL_LIGHTNING, FALSE);
	} else {
		learn_talent(ch, TAL_LIGHTNING);
	        magic_damage(victim, ch,
        	(TALENT(ch, TAL_MIND) + TALENT(ch, TAL_LIGHTNING))*2,
        	MAG_ELECTRICITY, TAL_MIND, FALSE);
	}
}
...
void do_smite(CHAR_DATA *ch, char *argument) {
        ...
        act(AT_WHITE, "Holy lightning erupts from your $t!",
                ch, hands(ch), NULL, TO_CHAR);
        act(AT_WHITE, "Holy lightning erupts from $n's $t!",
                ch, hands(ch), NULL, TO_ROOM);
        ...
        if (IS_AFFECTED(victim, AFF_UNHOLY)
        ||  IS_AFFECTED(victim, AFF_UNDEAD)
        ||  IS_AFFECTED(victim, AFF_DEMON)) {
        	magic_damage(victim, ch,
        	TALENT(ch, TAL_HEALING)*3 + (TALENT(ch, TAL_LIGHTNING)
		+ TALENT(ch, TAL_SECURITY))*2,
  		MAG_HOLY, TAL_HEALING, FALSE);
		learn_talent(ch, TAL_LIGHTNING);
		learn_talent(ch, TAL_SECURITY);
        } else {
        	magic_damage(victim, ch, TALENT(ch, TAL_LIGHTNING)*2,
        	MAG_ELECTRICITY, TAL_LIGHTNING, FALSE);
        }
}
...
void do_holyblast(CHAR_DATA *ch, char *argument) {
        ...
        if (ch->curr_talent[TAL_HEALING]
	+   ch->curr_talent[TAL_SECURITY] < 200 && !IS_NPC(ch)) {
                huh(ch);
                return;
        }

        act(AT_WHITE, "Holy energy blasts from your $t!",
                ch, hands(ch), NULL, TO_CHAR);
        act(AT_WHITE, "Holy energy blasts from $n's $t!",
                ch, hands(ch), NULL, TO_ROOM);
	...
	||  IS_AFFECTED(victim, AFF_DEMON)) {
	        magic_damage(victim, ch,
		(TALENT(ch, TAL_HEALING) + TALENT(ch, TAL_SECURITY))*2,
	        MAG_HOLY, TAL_HEALING, FALSE);
		learn_talent(ch, TAL_SECURITY);
	} else {
...
void do_lightning(CHAR_DATA *ch, char *argument) {
	...
	int amt;
        ...
        act(AT_YELLOW, "A bolt of lightning flies from your $t!",
                ch, hands(ch), NULL, TO_CHAR);
        act(AT_YELLOW, "A bolt of lightning flies from $n's $t!",
                ch, hands(ch), NULL, TO_ROOM);
...
bullet = create_object( get_obj_index( 50 ), 0 ); /* blast */
	...
	amt = number_range(TALENT(ch, TAL_LIGHTNING),
	TALENT(ch, TAL_LIGHTNING) * 5);
...
void do_chill(CHAR_DATA *ch, char *argument) {
        ...
        act(AT_FROST, "Frigid snow flies from your $t!",
                ch, hands(ch), NULL, TO_CHAR);
        act(AT_FROST, "Frigid snow flies from $n's $t!",
                ch, hands(ch), NULL, TO_ROOM);

        victim = find_target(ch, argument, TRUE);
        learn_talent(ch, TAL_WATER);
        magic_damage(victim, ch, TALENT(ch, TAL_FROST) +
        TALENT(ch, TAL_WATER),
        MAG_COLD, TAL_FROST, FALSE);
}
...
void do_frostbite(CHAR_DATA *ch, char *argument) {
        ...
        if (IS_NPC(ch) || ch->curr_talent[TAL_FROST]
	+   ch->pcdata->weapon[SK_FANG] < 150) {
                huh(ch);
                return;
        }

        act(AT_FROST, "You snap your teeth with the chill of frost!",
                ch, NULL, NULL, TO_CHAR);
        act(AT_FROST, "$n snaps $s glistening icy teeth!",
                ch, NULL, NULL, TO_ROOM);
...
void do_taint(CHAR_DATA *ch, char *argument) {
	...
	AFFECT_DATA af;
        ...
        act(AT_DGREEN, "Noxious fumes burst from your $t!",
                ch, hands(ch), NULL, TO_CHAR);
        act(AT_DGREEN, "Noxious fumes burst from $n's $t!",
                ch, hands(ch), NULL, TO_ROOM);
...
void do_splash(CHAR_DATA *ch, char *argument) {
	...
	if (ch->curr_talent[TAL_EARTH] < 1) {
        act(AT_BLUE, "Water shoots from your $t!",
                ch, hands(ch), NULL, TO_CHAR);
        act(AT_BLUE, "Water shoots fly from $n's $t!",
                ch, hands(ch), NULL, TO_ROOM);
	} else {
        act(AT_GREEN, "Acid sprays from your $t!",
                ch, hands(ch), NULL, TO_CHAR);
        act(AT_GREEN, "Acid sprays from $n's $t!",
                ch, hands(ch), NULL, TO_ROOM);
	}
	...
	magic_damage(victim, ch,
	TALENT(ch, TAL_EARTH) + TALENT(ch, TAL_WATER),
	MAG_ACID, TAL_WATER, FALSE);
}
...
void do_distort(CHAR_DATA *ch, char *argument) {
        ...
        ||  ch->curr_talent[TAL_VOID] > 0
        ||  ch->curr_talent[TAL_TIME] > 0) {
                act(AT_FIRE, "Ripples of distortion radiate from your $t!",
                        ch, hands(ch), NULL, TO_CHAR);
                act(AT_FIRE, "Ripples of distortion radiate from $n's $t!",
                        ch, hands(ch), NULL, TO_ROOM);
        }
	...
	if (ch->curr_talent[TAL_CHANGE] > 0)
	        magic_damage(victim, ch, TALENT(ch, TAL_CHANGE),
	        MAG_CHANGE, TAL_CHANGE, FALSE);
	if (ch->curr_talent[TAL_TIME] > 0)
		magic_damage(victim, ch, TALENT(ch, TAL_TIME),
		MAG_CHANGE, TAL_TIME, FALSE);
	if (ch->curr_talent[TAL_VOID] > 0)
	        magic_damage(victim, ch, TALENT(ch, TAL_VOID),
		MAG_CHANGE, TAL_VOID, FALSE);
}
...
void do_nightmare(CHAR_DATA *ch, char *argument) {
	...
	if (!IS_AFFECTED(ch, AFF_DREAMWORLD)) {
        ...
        magic_damage(victim, ch, 2 * (TALENT(ch, TAL_DREAM) +
	TALENT(ch, TAL_MIND) + TALENT(ch, TAL_ILLUSION)),
...
void do_shock(CHAR_DATA *ch, char *argument) {
        ...
        act(AT_YELLOW, "Lightning crackles from your $t!",
                ch, hands(ch), NULL, TO_CHAR);
        act(AT_YELLOW, "Lightning crackles from $n's $t!",
                ch, hands(ch), NULL, TO_ROOM);
	...
	if (IS_UNDERWATER(ch) || ch->in_room->curr_water > 30) {
		act(AT_YELLOW, "The electricity surges alarmingly through the water!",
			ch, NULL, NULL, TO_CHAR);
		act(AT_YELLOW, "$n's electricity surges alarmingly through the water!",
			ch, NULL, NULL, TO_ROOM);
		if (!IS_NPC(ch))
		magic_damage(ch, ch, TALENT(ch, TAL_LIGHTNING),
		MAG_ELECTRICITY, TAL_LIGHTNING, FALSE);
		magic_damage(victim, ch, TALENT(ch, TAL_LIGHTNING)*2,
		MAG_ELECTRICITY, TAL_LIGHTNING, FALSE);
	} else {
...
void do_storm(CHAR_DATA *ch, char *argument) {
	...
	if (ch->curr_talent[TAL_WIND] < 10 && !IS_NPC(ch)) {
        ...
        if (ch->in_room->curr_water >= 100) {
                act(AT_CYAN, "There isn't enough air here.",
                        ch, NULL, NULL, TO_CHAR);
                act(AT_CYAN, "Some air bubbles float up toward the surface.",
                        ch, NULL, NULL, TO_ROOM);
                return;
        }
	...
	if (last_attack == TAL_LIGHTNING) {
	   last_attack = TAL_CATALYSM;
           if (ch->curr_talent[TAL_CATALYSM] > 0) {
                magic_damage(victim, ch, TALENT(ch, TAL_CATALYSM),
                MAG_ENERGY, TAL_CATALYSM, FALSE);
		continue;
	   }
	}
...
void do_wind(CHAR_DATA *ch, char *argument) {
	...
	if (ch->in_room->curr_water >= 100) {
        ...
        act(AT_CYAN, "Strong winds blow from your $t!",
                ch, hands(ch), NULL, TO_CHAR);
        act(AT_CYAN, "Strong winds blow from $n's $t!",
                ch, hands(ch), NULL, TO_ROOM);
	...
	magic_damage(victim, ch, 
	(TALENT(ch, TAL_WIND)*2 + ch->in_room->area->weather->wind)/3,
	MAG_WIND, TAL_WIND, FALSE);
}
...
void do_vines(CHAR_DATA *ch, char *argument) {
	...
	magic_damage(victim, ch,
	(TALENT(ch, TAL_EARTH)*2 + ch->in_room->curr_vegetation)/3,
...
void do_blast(CHAR_DATA *ch, char *argument) {
	...
	if (ch->curr_talent[TAL_CATALYSM] > 0
	||  ch->curr_talent[TAL_FIRE] > 0
	||  ch->curr_talent[TAL_LIGHTNING] > 0) {
		act(AT_FIRE, "A blast of energy bursts from your $t!",
			ch, hands(ch), NULL, TO_CHAR);
		act(AT_FIRE, "A blast of energy bursts from $s $t!",
			ch, hands(ch), NULL, TO_ROOM);
	}
...
void do_mindflay(CHAR_DATA *ch, char *argument) {
	...
	if ((ch->curr_talent[TAL_MIND] > 40 || IS_NPC(ch))
	&& !IS_SET(victim->resistant, RIS_PSIONIC))
		victim->mental_state +=
		number_range(0, ch->curr_talent[13] / 10);

	if (ch->curr_talent[TAL_MIND] >= 1)
		magic_damage(victim, ch,
		TALENT(ch, TAL_MIND) + victim->perm_int - victim->perm_wis,
		MAG_PSIONIC, TAL_MIND, FALSE);
	if (ch->curr_talent[TAL_DREAM] >= 1)
		magic_damage(victim, ch, TALENT(ch, TAL_DREAM),
		MAG_DREAM, TAL_DREAM, FALSE);
	if (ch->curr_talent[TAL_ILLUSION] >= 1)
		magic_damage(victim, ch, TALENT(ch, TAL_ILLUSION),
		MAG_LIGHT, TAL_ILLUSION, FALSE);
}
...
void do_drain(CHAR_DATA *ch, char *argument) {
	...
	int amt;
        ...
        if (ch->curr_talent[TAL_DEATH] < 1 && !IS_NPC(ch)
	&&  ch->curr_talent[TAL_CATALYSM] < 1 
	&&  ch->curr_talent[TAL_HEALING] < 1) {
                huh(ch);
                return;
        }
	...
	if (IS_AFFECTED(victim, AFF_UNDEAD)) {
		...
		magic_damage(ch, victim,
		TALENT(ch, TAL_DEATH) + TALENT(ch, TAL_HEALING),
		MAG_DRAIN, TAL_DEATH, TRUE);
	    }
		amt = number_range(TALENT(ch, TAL_DEATH) +
		TALENT(ch, TAL_HEALING),
		(TALENT(ch, TAL_DEATH) +
		TALENT(ch, TAL_HEALING))*10);
		use_magic(ch, TAL_DEATH, amt);
		learn_talent(ch, TAL_HEALING);
        	if (!IS_SET(ch->immune, RIS_DRAIN)
		|| ch == victim) {
			act(AT_MAGIC, "Your undead flesh repairs itself.",
				victim, NULL, NULL, TO_CHAR);
			act(AT_MAGIC, "$n's undead flesh repairs itself.",
				victim, NULL, NULL, TO_ROOM);
			if (IS_AFFECTED(ch, AFF_UNHOLY)) amt *= 2;
			if (IS_AFFECTED(ch, AFF_HOLY) && amt != 0) amt /= 2;
               		victim->hit += amt;
                	if (victim->hit > victim->max_hit)
        	                victim->hit = victim->max_hit;
        for (part = victim->first_part;part;part = part->next) {
	...
	magic_damage(victim, ch,
	TALENT(ch, TAL_DEATH) + TALENT(ch, TAL_HEALING),
	MAG_DRAIN, TAL_DEATH, FALSE);
       	amt = number_range(1, TALENT(ch, TAL_DEATH) + TALENT(ch, TAL_HEALING));
	use_magic(ch, TAL_HEALING, amt);
	if (!IS_SET(victim->immune, RIS_DRAIN)
	&&  TALENT(victim, TAL_DEATH) < 100) {
		if (IS_AFFECTED(ch, AFF_UNHOLY)) amt *= 2;
		if (IS_AFFECTED(victim, AFF_UNHOLY)) amt *= 2;
		if (IS_AFFECTED(ch, AFF_HOLY) && amt != 0) amt /= 2;
		if (IS_AFFECTED(victim, AFF_HOLY) && amt != 0) amt /= 2;
        	ch->hit += amt;
...
void travel_teleport(CHAR_DATA *ch) {
    ...
    if (IS_NPC(ch) && xIS_SET(ch->act, ACT_SENTINEL)) return;
    ...
    act( AT_MAGIC, "Reality seems to twist for a moment, and your surroundings warp and change.", ch,
	NULL, NULL, TO_CHAR);
    act( AT_MAGIC, "Reality seems to twist for a moment, and $n vanishes.", ch,
	NULL, NULL, TO_ROOM);
...
void do_travel(CHAR_DATA *ch, char *argument) {
	...
	|| xIS_SET(ch->act, ACT_SENTINEL))) return;
	...
	if (!pRoomIndex) {
        ...
        use_magic(ch, TAL_MOTION, (250 - ch->curr_talent[TAL_MOTION]*2)
        * (ch->mount ? 2 : 1));
...
void do_summon(CHAR_DATA *ch, char *argument) {
    ...
    if (victim->in_room) char_from_room(victim);
    victim->in_obj = NULL;
...
void do_morph(CHAR_DATA *ch, char* argument) {
	...
	if (argument && !ch->pcdata->were_race) {
		/* if they dont have a morph, set it to the argument */
		...
		if (has_bodypart(ch, BP_RHAND)
		&& IS_SET(newrace->parts, PART_HANDS)) {
	...
	if (IS_AFFECTED(ch, AFF_ANTHRO))
        ...
        act( AT_MAGIC, "Your body changes and warps, and you become $t.",
                ch, aoran(newrace->name), NULL, TO_CHAR);
        act( AT_MAGIC, "$n's body changes and warps, and $e becomes $t.",
                ch, aoran(newrace->name), NULL, TO_ROOM);
...
void do_change(CHAR_DATA *ch, char* argument) {
	...
	if (str_cmp(newrace->species, victim->nation->species)
	...
	species = find_species(newrace->species);
        ...
        ch_printf(victim, "Your body changes and warps, and you become %s.\n\r",
                aoran(newrace->name));
        act( AT_MAGIC, "$N's body changes and warps, and $E becomes $t.",
                ch, aoran(newrace->name), victim, TO_NOTVICT);
	if (ch != victim) {
		act( AT_MAGIC, "You change and warp $N's body into $t.",
			ch, aoran(newrace->name), victim, TO_CHAR);
	}
...
void do_alter(CHAR_DATA *ch, char *argument) {
	...
	&&        !str_cmp(victim->pcdata->skin_type, arg2)) {
	...
	&&        !str_cmp(victim->pcdata->extra_type, arg2)) {
...
bool check_antimagic( CHAR_DATA *ch, CHAR_DATA *victim )
{
    ...
    if ( number_range(1,105) >
        (number_range(1, TALENT(victim, TAL_VOID) )))
    {
        return FALSE;
    }
    ...
    act( AT_MAGIC, "You cast $n's magic into the Void.",
        ch, NULL, victim, TO_VICT );
    act( AT_MAGIC, "$N channels antimagic, stopping your attack.",
        ch, NULL, victim, TO_CHAR );
    act( AT_MAGIC, "$N channels antimagic, stopping $n's attack.",
        ch, NULL, victim, TO_NOTVICT );
...
bool check_blur( CHAR_DATA *ch, CHAR_DATA *victim )
{
    ...
    if ( number_range(1,200) >
        (number_range(1, TALENT(victim, TAL_TIME) - victim->encumberance)))
    {
        return FALSE;
    }
    ...
    act( AT_MAGIC, "$n swings at you, but you blur out of the way.",
        ch, NULL, victim, TO_VICT );
    act( AT_MAGIC, "You swing at $N, but $E blurs out of the way.",
        ch, NULL, victim, TO_CHAR );
    act( AT_MAGIC, "$n swings at $N, but $E blurs out of the way.",
        ch, NULL, victim, TO_NOTVICT );
...
bool check_displacement( CHAR_DATA *ch, CHAR_DATA *victim )
{
    ...
    if ( number_range(1,100 + get_curr_per(ch)) >
	(number_range(1, TALENT(victim, TAL_ILLUSION) )))
    {
        return FALSE;
    }
    ...
    act( AT_MAGIC, "$n strikes at your displaced image, missing you.",
	ch, NULL, victim, TO_VICT );
    act( AT_MAGIC, "You strike at $N's displaced image, missing $M.",
	ch, NULL, victim, TO_CHAR );
    act( AT_MAGIC, "$n strikes at $N's displaced image, missing $M.",
	ch, NULL, victim, TO_NOTVICT );
...
void do_blink(CHAR_DATA *ch, char *argument) {
	...
	act(AT_MAGIC, "You blink a short distance away.",
		ch, NULL, NULL, TO_CHAR);
	act(AT_MAGIC, "$n blinks out of existance.",
		ch, NULL, NULL, TO_ROOM);
...
void do_cure(CHAR_DATA *ch, char *argument) {
    ...
    else
    {
      ...
      } else if (!str_cmp(arg, "rot")
        && (ch->curr_talent[TAL_HEALING] >= 60)) {
    if ( !is_affected( victim, gsn_mummy_rot ) )
    {
        if ( ch != victim )
    ...
    if ( ch != victim )
      ...
      } else if (!str_cmp(arg, "plague")
        && (ch->curr_talent[TAL_HEALING] >= 75)) {
    if ( !is_affected( victim, gsn_disease ) )
    {
        if ( ch != victim )
	...
	&& ch->curr_talent[TAL_SECURITY] >= 35) {
    if ( is_affected( victim, gsn_curse ) )
    {
        affect_strip( victim, gsn_curse );
        set_char_color( AT_MAGIC, victim );
	...
	} else if (!str_cmp(arg, "corrupt")
	&& (ch->curr_talent[TAL_HEALING] >= 35
	||  ch->curr_talent[TAL_CHANGE] >= 50)) {
    if ( is_affected( victim, gsn_corruption ) )
    {
        affect_strip( victim, gsn_corruption );
        set_char_color( AT_MAGIC, victim );
	...
	} else if (!str_cmp(arg, "weak")
	&& ch->curr_talent[TAL_HEALING] >= 10) {
    if ( is_affected( victim, gsn_weaken ) )
    {
        affect_strip( victim, gsn_weaken );
        set_char_color( AT_MAGIC, victim );
        ...
        } else if (!str_cmp(arg, "drunk")
        && ch->curr_talent[TAL_MIND] + ch->curr_talent[TAL_HEALING] >= 20) {
  		if ( victim->pcdata )
  		  victim->pcdata->condition[COND_DRUNK] = 0;
...
void do_dispel(CHAR_DATA *ch, char *argument) {
	...
	if (ch->curr_talent[TAL_CATALYSM] < 10
	&&  ch->curr_talent[TAL_VOID] < 10) {
		huh(ch);
		return;
	}

	if (!str_cmp(argument, "room")
	&& ch->curr_talent[TAL_CATALYSM] >= 50) {
	...
	&&  ch->curr_talent[TAL_VOID] < 30) {
        ...
        if (IS_AFFECTED(victim, AFF_MAGICAL)) {
                ...
                + TALENT(ch, TAL_VOID)*5,
                MAG_ANTIMAGIC, TAL_CATALYSM, FALSE);
        } /* fall through to get their affs too */

	use_magic(ch, TAL_CATALYSM, 200 - ch->curr_talent[TAL_CATALYSM]);

	if (TALENT(victim, TAL_SECURITY) > TALENT(ch, TAL_CATALYSM)
	+ TALENT(ch, TAL_VOID)
	&& ch != victim) {
	  ...
	  if (ch == victim)
	     ...
	     send_to_char("They dont seem to be affected by any enchantments.\n\r", ch);
      }
	} /* end player dispel */
	else {
...
void do_rune(CHAR_DATA *ch, char *argument) {
	...
	if (IS_SET(ch->in_room->room_flags, ROOM_NO_MAGIC)) {
        ...
        if (!str_cmp(argument, "life")
        && ch->curr_talent[TAL_HEALING] >= 100) {
		...
		tal = TAL_HEALING;
	} else if (!str_cmp(argument, "distract")
	&& ch->curr_talent[TAL_DREAM] >= 100) {
        ...
        && ch->curr_talent[TAL_SECURITY] >= 100) {
...
void do_shield(CHAR_DATA *ch, char *argument) {
	...
	if (!str_cmp(argument, "none")
	&& ch->shield) {
		act(AT_MAGIC, "You lower your magical shield.",
			ch, NULL, NULL, TO_CHAR);
		act(AT_MAGIC, "$n lowers $s magical shield.",
			ch, NULL, NULL, TO_ROOM);
		ch->shield = MAG_NONE;
		return;
        } else if (!str_cmp(argument, "acid")
	...
	&& ch->curr_talent[TAL_EARTH] >= 10) {
        ...
        && ch->curr_talent[TAL_CATALYSM] >= 30
	&& ch->curr_talent[TAL_EARTH] >= 10) {
                act(AT_WHITE, "Dozens of dancing daggers twirl around you.",
                        ch, NULL, NULL, TO_CHAR);
                act(AT_WHITE, "Dozens of dancing daggers twirl around $n.",
                        ch, NULL, NULL, TO_ROOM);
                ch->shield = MAG_PIERCE;
                tal = TAL_CATALYSM;
        } else if (!str_cmp(argument, "dream")
        && ch->curr_talent[TAL_DREAM] >= 40) {
	...
	&& ch->curr_talent[TAL_FIRE] >= 40) {
        ...
        && ch->curr_talent[TAL_ILLUSION] >= 40) {
        ...
        && ch->curr_talent[TAL_CATALYSM] >= 60) {
        ...
        && ch->curr_talent[TAL_DEATH] >= 40) {
                ...
                tal = TAL_DEATH;
        } else if (!str_cmp(argument, "shock")
        && ch->curr_talent[TAL_LIGHTNING] >= 40) {
        ...
        && ch->curr_talent[TAL_SPEECH] >= 40) {
                ...
                ch->shield = MAG_SOUND;
                tal = TAL_SPEECH;
        } else if (!str_cmp(argument, "stone")
        && ch->curr_talent[TAL_EARTH] >= 40) {
        ...
        && ch->curr_talent[TAL_CATALYSM] >= 10
	&& ch->curr_talent[TAL_EARTH] >= 30) {
                act(AT_WHITE, "A dozen dancing swords spin around you.",
                        ch, NULL, NULL, TO_CHAR);
                act(AT_WHITE, "A dozen dancing swords spin around $n.",
                        ch, NULL, NULL, TO_ROOM);
                ch->shield = MAG_SLASH;
                tal = TAL_EARTH;
        } else if (!str_cmp(argument, "thought")
        && ch->curr_talent[TAL_MIND] >= 40) {
                ...
                tal = TAL_MIND;
        } else if (!str_cmp(argument, "vines")
        && ch->curr_talent[TAL_EARTH] >= 40) {
        ...
        && (ch->curr_talent[TAL_CHANGE] >= 40
	||  ch->curr_talent[TAL_TIME] >= 40
	||  ch->curr_talent[TAL_VOID] >= 40)) {
...
void do_bind(CHAR_DATA *ch, char *argument) {
	...
	act(AT_BLUE, "$T $p, binding it with power.",
		ch, obj,talent->message, TO_ROOM);
...
void do_brand(CHAR_DATA *ch, char *argument) {
	...
	&& ch->curr_talent[TAL_LIGHTNING] >= 50) {
	...
	&& ch->curr_talent[TAL_WATER] >= 50) {
		...
		tal = TAL_WATER;
        } else if (!str_cmp(argument, "antimatter")
        && ch->curr_talent[TAL_VOID] >= 60) {
                ...
                tal = TAL_VOID;
        } else if (!str_cmp(argument, "sonic")
        && ch->curr_talent[TAL_SPEECH] >= 50) {
	...
	+  ch->curr_talent[TAL_VOID] >= 60) {
                ...
                tal = TAL_CHANGE;
	} else if (!str_cmp(argument, "venom")
	&& ch->curr_talent[TAL_DEATH] >= 50) {
		...
		tal = TAL_DEATH;
	} else if (!str_cmp(argument, "vorpal")
	&& ch->curr_talent[TAL_CATALYSM] >= 60) {
	   ...
	   && obj->value[5] != SK_CLAW
		...
		tal = TAL_CATALYSM;
	} else if (!str_cmp(argument, "storm")
	&& ch->curr_talent[TAL_WIND] >= 60) {
		...
		tal = TAL_WIND;
	} else if (!str_cmp(argument, "radiant")
	&& ch->curr_talent[TAL_ILLUSION] >= 50) {
	...
	&& ch->curr_talent[TAL_EARTH] >= 60) {
        if (obj->value[5] != SK_MACE
        	...
        	act( AT_DGREY, "$p becomes harder than rock!",
                ch, obj, NULL, TO_CHAR );
        	act( AT_DGREY, "$n's $p becomes harder than rock!",
                ch, obj, NULL, TO_ROOM);
	        obj->value[4] = BRAND_CRUSH;
                tal = TAL_EARTH;
        } else if (!str_cmp(argument, "drain")
        && ch->curr_talent[TAL_DEATH] >= 60) {
                ...
                tal = TAL_DEATH;
	} else if (!str_cmp(argument, "psionic")
	&& ch->curr_talent[TAL_MIND] >= 50) {
		act( AT_PURPLE, "A strange humming sound emanates from $p!",
			ch, obj, NULL, TO_CHAR);
		act( AT_PURPLE, "A strange humming sound emanates from $n's $p!",
			ch, obj, NULL, TO_ROOM);
...
if ( victim == ch
    ...
    act( AT_MAGIC, "You shift into another dream.", ch, NULL, NULL,
TO_CHAR);
	...
	} else if (!str_cmp(arg, "watch")
	&& ch->curr_talent[TAL_DREAM] >= 25) {
      ...
      if (d->connected == CON_PLAYING)
	   if (nifty_is_name(d->character->name, corpse->name)) {
		...
		if (ch->mana < victim->perm_con*10) {
			send_to_char("You dont have enough energy to resurrect this corpse.\n\r", ch);
			return;
		}
...
void do_identify( CHAR_DATA *ch, char *argument )
{
  ...
  if ( ( victim = get_char_room( ch, argument ) ) == NULL )
  {
   ...
   ch_printf( ch, "&Oand %d copper coins&C.\n\r", gp);
	  ...
	  {
		if (obj->timer == 0) ch_printf( ch,
			"It will not decay.\n\r");
		else ch_printf(ch,
			"It will last another %d hours.\n\r",
			obj->timer);
	  }
    ...
    case ITEM_STAFF:
	if (ch->curr_talent[TAL_SEEKING] >= 20) {
        ch_printf( ch, "Has %d out of %d charges left of",
            obj->value[1], obj->value[2] );
    ...
    case ITEM_RADIO:
	ch_printf( ch,
	"The volume is set at %d.\n\r",
		obj->value[0]);
	ch_printf( ch,
	"The frequency is set at %d.\n\r",
		obj->value[1]);
	break;
    }
    ...
    if (!victim->first_affect)
    {
      ...
      for ( paf = victim->first_affect; paf; paf = paf->next )
      {
		...
		&& TALENT(ch, TAL_SEEKING) > TALENT(victim, TAL_SEEKING)) {
			...
			else if (victim->talent[talent->index] >= 75
...
void do_enchant( CHAR_DATA *ch, char *argument )
{
	...
	if (!str_cmp(argument, "magic")) {
		...
		&&  TALENT(ch, TAL_MOTION) < 50) {
                        send_to_char("You can't enchant somthing to return when thrown.\n\r", ch);
                        return;
                }
		if (obj->mana < amt) {
        ...
        } else if (!str_cmp(argument, "breath")
	&&     ch->curr_talent[TAL_WATER] >= 40) {
		tal = TAL_WATER;
        ...
        if (found==FALSE) {
                ...
                act(AT_MAGIC, "You imbue $p with magical breathing.",
                        ch, obj, NULL, TO_CHAR);
        }
        ...
        if (found==FALSE) {
                ...
                paf->next = NULL;
                LINK(paf, obj->first_affect, obj->last_affect, next, prev);
                act(AT_MAGIC, "You imbue $p with the power of flight.",
                        ch, obj, NULL, TO_CHAR);
        }

        } else if (!str_cmp(argument, "nightvision")
        &&     ch->curr_talent[TAL_SEEKING] >= 35) {
                tal = TAL_SEEKING;
        ...
        if (found==FALSE) {
                ...
                paf->next = NULL;
                LINK(paf, obj->first_affect, obj->last_affect, next, prev);
                act(AT_MAGIC, "You imbue $p with nightvision.",
                        ch, obj, NULL, TO_CHAR);
        }
	...
	&&  ch->curr_talent[TAL_DEATH] >= 10) {
		...
		obj->value[2] += 5;
		act(AT_MAGIC, "You imbue $p with the power of Death.",
			ch, obj, NULL, TO_CHAR);
	} else if (!str_cmp(argument, "weight")) {
		if (obj->weight > 1 && TALENT(ch, tal) / 10
      ...
      for (paf = obj->first_affect; paf; paf=paf->next) {
                ...
                if (TALENT(ch, tal) / 10 > paf->modifier) {
           		act(AT_MAGIC, "You enchant $p with the strength of the Earth.",
			ch, obj, NULL, TO_CHAR);
                        paf->modifier++;
                } else {
      ...
      for (paf = obj->first_affect; paf; paf=paf->next) {
                ...
                if (TALENT(ch, tal) / 10
                > paf->modifier) {
           act(AT_MAGIC, "You enchant $p with the strength of the Ice.",
		ch, obj, NULL, TO_CHAR);
                        paf->modifier++;
                } else {
           	send_to_char("You are not strong enough to further", ch);
      ...
      for (paf = obj->first_affect; paf; paf=paf->next) {
                ...
                if (TALENT(ch, tal) / 10
                > paf->modifier) {
           act(AT_MAGIC, "You enchant $p with the strength of the Flames.",
		ch, obj, NULL, TO_CHAR);
                        paf->modifier++;
                } else {
           	send_to_char("You are not strong enough to further", ch);
      ...
      for (paf = obj->first_affect; paf; paf=paf->next) {
                ...
                if (TALENT(ch, tal) / 10 > paf->modifier) {
           		act(AT_MAGIC, "You enchant $p with the power of Seeking.",
			ch, obj, NULL, TO_CHAR);
                        paf->modifier++;
                } else {
	           	send_to_char("You are not strong enough to further", ch);
      ...
      for (paf = obj->first_affect; paf; paf=paf->next) {
                ...
                if (TALENT(ch, tal) / 10
                > paf->modifier) {
           act(AT_MAGIC, "You enchant $p with the speed of the Wind.",
		ch, obj, NULL, TO_CHAR);
                        paf->modifier++;
                } else {
           	send_to_char("You are not strong enough to further", ch);
      ...
      for (paf = obj->first_affect; paf; paf=paf->next) {
                ...
                if (TALENT(ch, tal) / 10
                > paf->modifier) {
           act(AT_MAGIC, "You enchant $p with the power of Water.",
		ch, obj, NULL, TO_CHAR);
                        paf->modifier++;
                } else {
           	send_to_char("You are not strong enough to further", ch);
      ...
      for (paf = obj->first_affect; paf; paf=paf->next) {
                ...
                if (TALENT(ch, tal) / 10
                > paf->modifier) {
           act(AT_MAGIC, "You enchant $p with the power of the Mind.",
		ch, obj, NULL, TO_CHAR);
                        paf->modifier++;
                } else {
           	send_to_char("You are not strong enough to further", ch);
      ...
      for (paf = obj->first_affect; paf; paf=paf->next) {
                ...
                if (TALENT(ch, tal) / 10
                > paf->modifier) {
           act(AT_MAGIC, "You enchant $p with the strength of the Storm.",
		ch, obj, NULL, TO_CHAR);
                        paf->modifier++;
                } else {
           	send_to_char("You are not strong enough to further", ch);
      ...
      for (paf = obj->first_affect; paf; paf=paf->next) {
                ...
                if (TALENT(ch, tal) / 10
                > paf->modifier) {
           act(AT_MAGIC, "You enchant $p with the speed of Motion.",
		ch, obj, NULL, TO_CHAR);
                        paf->modifier++;
                } else {
           	send_to_char("You are not strong enough to further", ch);
        ...
        } else if (!str_cmp(argument, "container")
        ...
        &&  ch->curr_talent[TAL_VOID] >= 10)) {
                ...
                act(AT_MAGIC, "You imbue $p with the power of the Void.",
                        ch, obj, NULL, TO_CHAR);
	} else {
...
void do_preserve( CHAR_DATA *ch, char *argument )
{
	...
	+ TALENT(ch, TAL_FROST) + TALENT(ch, TAL_TIME)
	+ TALENT(ch, TAL_VOID) + TALENT(ch, TAL_CATALYSM);
...
void do_reshape(CHAR_DATA *ch, char *argument) {
	...
	argument = one_argument(argument, arg);
        ...
        lvl = ch->curr_talent[TAL_CHANGE]
        +   ch->curr_talent[TAL_CATALYSM]
        +   ch->curr_talent[TAL_ILLUSION];
        if (lvl < 95) {
...
void do_void(CHAR_DATA *ch, char *argument) {
	...
	if (IS_NPC(victim))
		...
		do_look(victim, "auto");
		return;
	} else if (!str_cmp(arg, "extract")
	&& ch->curr_talent[TAL_VOID] >= 45) {
	...
	if (victim == ch) {
		send_to_char("That was a shoft trip!\n\r", ch);
		return;
	}
	...
	} else if (!str_cmp(arg, "rift")
	&& ch->curr_talent[TAL_VOID] >= 80) {
...
void do_decay( CHAR_DATA *ch, char *argument )
{
	...
	+ ch->curr_talent[TAL_CATALYSM] + ch->curr_talent[TAL_DEATH]
	+ ch->curr_talent[TAL_VOID]) / 2;
...
void do_immobilize(CHAR_DATA *ch, char *argument) {
	...
	lvl = ch->curr_talent[TAL_EARTH] + ch->curr_talent[TAL_TIME]
	...
	if (lvl < 20) {
        ...
        ch->curr_talent[TAL_FROST]*10 - 400) {
                act(AT_FROST, "It isn't cold enough here.",
                        ch, NULL, NULL, TO_CHAR);
                return;
        }
	...
	100 - ch->curr_talent[TAL_EARTH]) {
		act(AT_GREEN, "There isn't enough vegetatation here.",
			ch, NULL, NULL, TO_CHAR);
		return;
	}
