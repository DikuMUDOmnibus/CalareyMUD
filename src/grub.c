...
void gr_init (void)
{
  ...
  strcpy(gr_fd[ 7].nam, "site"   ); gr_fd[ 7].num=FALSE;
  strcpy(gr_fd[ 8].nam, "last"   ); gr_fd[ 8].num=TRUE;
  strcpy(gr_fd[ 9].nam, "pkill"  ); gr_fd[ 9].num=FALSE;
}
  ...
  while ( !feof(fp) )                       /* read each input record    */
  {
     ...
     if ( res )                             /* record is a match         */
     {
        ...
        if ( !title_sw && dis_num > 0 )     /* print title if applicable */
        {
           ...
           "Name", "Lv", "S", "R", "C", "Room", "Gold",
           "Site", "Last", "Pk");
           title_sw = TRUE;
        }
        if ( tot_match <= dis_num )         /* print record if applicable */
           ch_printf(ch,
  "%-12s %2hd %c %2s %c %5hd %11ld %-15s %6lu %c\n\r", 
  ...
  if ( obj_index_hash[hash] )
     for (pObj=obj_index_hash[hash]; pObj; pObj=pObj->next)
         if (pObj->layers > 0)
         {
            ...
            pager_printf(ch, "%4d %5d %5d   %s\n\r",
              cou, pObj->vnum, pObj->layers,
              pObj->short_descr);
          }
}
...
/*  
     ch_printf(ch, 
       "\n\rINDEX_DATA vnum=%d name=%s extra_flags=%d\n\r",
       px->vnum, px->name, px->extra_flags);
