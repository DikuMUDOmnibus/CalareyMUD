...
char *  const   rev_dir_name	[]		=
{
    ...
    "southwest", "southeast", "northwest", "northeast", "somewhere"
};
...
void drop_dream_items(CHAR_DATA *ch, OBJ_DATA *obj);
...
void explore_room(CHAR_DATA *ch, ROOM_INDEX_DATA *room) {
	...
	if (IS_NPC(ch)) return;
	if (ch->in_room->vnum - ch->in_room->area->low_r_vnum > 125)
		return;

	xarea = ch->pcdata->first_explored;
	while (xarea) {
		if (xarea->index == ch->in_room->area->low_soft_range) {
			if (xIS_SET(xarea->rooms, ch->in_room->vnum
			- ch->in_room->area->low_r_vnum)) return;
			xSET_BIT(xarea->rooms, ch->in_room->vnum
			- ch->in_room->area->low_r_vnum);
			ch->exp += 100000;
			return;
		}
		xarea = xarea->next;
	...
	xarea->index = ch->in_room->area->low_soft_range;
	xCLEAR_BITS(xarea->rooms);
...
bool will_fall( CHAR_DATA *ch, int fall )
{
	...
	if (char_died(ch)) return FALSE;
 
	if (IS_AFFECTED(ch, AFF_FLYING) &&
	(IS_NPC(ch) || xIS_SET(ch->pcdata->perm_aff, AFF_FLYING)) &&
	ch->curr_talent[TAL_WIND] < 30 &&
	ch->curr_talent[TAL_MOTION] < 60 &&
	(!can_use_bodypart(ch, BP_RWING) ||
	 !can_use_bodypart(ch, BP_LWING))) {
		act(AT_PLAIN, "You plummet like a rock.", ch, NULL, NULL, TO_CHAR);
		act(AT_PLAIN, "$n plummets like a rock.", ch, NULL, NULL, TO_ROOM);
		if (IS_NPC(ch))
			xREMOVE_BIT(ch->affected_by, AFF_FLYING);
		else
			xREMOVE_BIT(ch->pcdata->perm_aff, AFF_FLYING);
	}

	if (ch->in_obj && IS_OBJ_STAT(ch->in_obj, ITEM_HOVER))
		return FALSE; 
	
	if ( IS_SET( ch->in_room->room_flags, ROOM_NOFLOOR )
	&&   CAN_GO(ch, DIR_DOWN)) {
	/* check for swimming -keo */
	if (IS_UNDERWATER(ch)) {
	   if (ch->in_obj && 
		 IS_SET(ch->in_obj->value[2], VEHICLE_UNDERWATER))
		return FALSE;

	if (	!can_use_bodypart(ch, BP_RFIN) &&
		!can_use_bodypart(ch, BP_LFIN) &&
		!can_use_bodypart(ch, BP_DFIN) &&
	   (( number_percent( ) >= (IS_NPC(ch) ? 100 :
	   LEARNED(ch, gsn_swim ) + (15-encumbrance(ch, 10)) 
	   + ch->curr_talent[TAL_WATER] ) ))) {
		learn_from_failure(ch, gsn_swim);
                set_char_color( AT_FALLING, ch );
                send_to_char( "You're sinking down...\n\r", ch );
                move_char( ch, get_exit(ch->in_room, DIR_DOWN), ++fall );
		if (ch->in_obj) {
			obj_from_room(ch->in_obj);
			obj_to_room(ch->in_obj, ch->in_room);
		}
		return TRUE;
	   } else {
	...
	if ( ch->in_obj || (!IS_AFFECTED( ch, AFF_FLYING )
...
{
    /* the billion vnum patch really screwed this up, the author didnt
       understand virtual rooms - shogar */
    /* >> and << dont work with long long so fake it out with a small rewrite*/
    EXIT_DATA *xit, *bxit;
    EXIT_DATA *orig_exit = (EXIT_DATA *) *pexit;
    ROOM_INDEX_DATA *room, *backroom;
    int brvnum;
    int serial;
    int virtual=in_room->virtual;
    int roomnum;
    int distance = -1;
    ...
    if ( virtual )	/* room is virtual */
    {
	serial = in_room->vnum;
	roomnum = in_room->tele_vnum;
	if ( (serial) == orig_exit->vnum )
        {
	  brvnum = virtual;
	  --roomnum;
	  distance = roomnum;
	...
	backroom = in_room;
/*
	serial = ((long long) UMAX( r1, r2 ) << 32L) 
		| (long long) UMIN( r1, r2 );
*/
	serial=r1 + 1048576000;
	virtual=r1;
	distance = orig_exit->distance - 1;
...
ch_ret move_char( CHAR_DATA *ch, EXIT_DATA *pexit, int fall )
{
    ...
    if ( !IS_NPC( ch ) )
    {
      if (IS_FIGHTING(ch)) {
        ...
        {
               send_to_char("Your bloodlust is too great!", ch);
               return rNONE;
        }
      }
    ...
    if ( IS_NPC(ch) && xIS_SET(ch->affected_by, AFF_MOUNTED) )
      return retcode;
	...
	||   IS_SET(pexit->to_room->runes, RUNE_DISTRACT) ) ) {
		act( AT_PLAIN, "Mobs can't go in there.", ch, NULL, NULL, TO_CHAR );
		return rNONE;
	}

	if (IS_SET(pexit->to_room->room_flags, ROOM_NO_DREAM)
	&& IS_AFFECTED(ch, AFF_DREAMWORLD)
	&& ch->curr_talent[TAL_DREAM] < 100 ) {
		act( AT_MAGIC, "A mysterious shield prevents you from entering there.", ch, NULL, NULL, TO_CHAR);
		return rNONE;
	}
	...
	if (ch->in_obj) {
		if (to_room->sector_type == SECT_INSIDE) {
			send_to_char("Your vehicle won't fit in there.\n\r", ch);
			return rNONE;
		}
		if (!IS_SET(ch->in_obj->value[2], VEHICLE_SPACE)
		&& to_room->sector_type == SECT_DUNNO) {
			send_to_char("Your vehicle cannot clear the atmosphere.\n\r", ch);
			return rNONE;
		}
		if ((to_room->sector_type == SECT_UNDERWATER
		||   to_room->sector_type == SECT_OCEANFLOOR)
		&& !IS_SET(ch->in_obj->value[2], VEHICLE_UNDERWATER)) {
			send_to_char("Your vehicle is not a submersible.\n\r", ch);
			return rNONE;
		}
		if (!IS_SET(ch->in_obj->value[2], VEHICLE_AIR)) {
		if (to_room->sector_type == SECT_SWAMP) {
			send_to_char("Your vehicle would become stuck in the mires.\n\r", ch);
			return rNONE;
		}
		if ((to_room->sector_type == SECT_MOUNTAIN
		||   to_room->sector_type == SECT_HILLS
		||   to_room->sector_type == SECT_FOREST)
		&& !IS_SET(ch->in_obj->value[2], VEHICLE_CLIMB)) {
			send_to_char("Your vehicle cannot handle that rough terrain.\n\r", ch);
			return rNONE;
		}
		else if ((to_room->sector_type == SECT_DESERT
		||   to_room->sector_type == SECT_BEACH)
		&& !IS_SET(ch->in_obj->value[2], VEHICLE_SAND)) {
			send_to_char("Your vehicle would founder in the sand.\n\r", ch);
			return rNONE;
		}
		else if ((to_room->sector_type == SECT_WATER_SWIM
		||  to_room->sector_type == SECT_WATER_NOSWIM)
		&&  !IS_SET(ch->in_obj->value[2], VEHICLE_SURFACE)) {
			send_to_char("Your vehicle cannot float on water.\n\r", ch);
			return rNONE;
		}
		else if ((to_room->sector_type == SECT_ICE
		||  IS_SET(to_room->room_flags, ROOM_SLIPPERY))
		&& !IS_SET(ch->in_obj->value[2], VEHICLE_ICE)) {
			send_to_char("Your vehicle would slip and spin out of control on the ice.\n\r", ch);
			return rNONE;
		}
		else if ((to_room->sector_type == SECT_CITY
		||  to_room->sector_type == SECT_FIELD)
		&& !IS_SET(ch->in_obj->value[2], VEHICLE_LAND)) {
			send_to_char("Your vehicle is not built to travel on land.\n\r", ch);
			return rNONE;
		}
		}
		if (to_room->sector_type == SECT_UNDERGROUND
		&&  !IS_SET(ch->in_obj->value[2], VEHICLE_UNDERGROUND)) {
			send_to_char("Your vehicle is not built for spelunking.\n\r", ch);
			return rNONE;
		}
	}
	...
	||   IS_SET( pexit->exit_info, EX_FLY )) && pexit->vdir == 4)
	{
	    if ( ch->in_obj && !IS_SET(ch->in_obj->value[2], VEHICLE_AIR))
	    {
		send_to_char("Your vehicle can't fly.\n\r", ch);
		return rNONE;
	    }
	    else if ( ch->mount && !IS_AFFECTED(ch, AFF_MOUNTED)
		&& !IS_AFFECTED( ch->mount, AFF_FLYING ) )
	    {
		send_to_char( "Your mount can't fly.\n\r", ch );
		return rNONE;
	    }
	    else if ( !ch->in_obj && !ch->mount && !IS_AFFECTED(ch, AFF_FLYING) )
	    {
	...
	if ( IS_SET( pexit->exit_info, EX_CLIMB ) )
	{
	    ...
	    found = FALSE;
	    if ( ch->in_obj && IS_SET(ch->in_obj->value[2], VEHICLE_CLIMB))
		found = TRUE;
	    else if ( ch->mount && !IS_AFFECTED(ch, AFF_MOUNTED)
	    && IS_AFFECTED( ch->mount, AFF_FLYING ) )
		found = TRUE;
	    ...
	    if ( !found && ch->in_obj ) {
		send_to_char("Your vehicle cannot traverse that steep terrain.\n\r", ch);
		return rNONE;
	    }
		...
		if (IS_UNDERWATER(ch)) {
		   ...
		   send_to_char( "You float lightly to the ground.\n\r", ch);
		   return rNONE;
		} else {
		   if (IS_AFFECTED(ch, AFF_MOUNTED)) throw_rider(ch);
		   set_char_color( AT_HURT, ch );
		   send_to_char( "OUCH! You hit the ground!\n\r", ch );
		   lose_hp(ch, 5);
			STRFREE(ch->last_taken);
			ch->last_taken = STRALLOC("stunned");
	...
	STRFREE(ch->last_taken);
	if ( ch->mount ) {
	  ch->last_taken = STRALLOC("riding");
	  ch->mount->move -= move;
	} else if (IS_UNDERWATER(ch)) {
	  ch->last_taken = STRALLOC("swimming");
	  ch->move -= move;
	} else {
	  ch->last_taken = STRALLOC("walking");
	  ch->move -= move;
	}
	WAIT_STATE(ch, move);
    }
    ...
    if  (!ch->in_obj && (ch->in_room->sector_type == SECT_UNDERWATER
     ||  ch->in_room->sector_type == SECT_OCEANFLOOR ))
	if ((ch->position <= POS_STANDING) && (!fall)) {

	if (ch->in_obj)
	if (!IS_SET(ch->in_obj->value[2], VEHICLE_UNDERWATER)) {
		send_to_char("Your vehicle won't go anywhere.\n\r", ch);
		return rNONE;
	}
    ...
    if (!can_use_bodypart(ch, BP_RFIN) &&
                !can_use_bodypart(ch, BP_LFIN) &&
                !can_use_bodypart(ch, BP_DFIN) &&
	(( number_percent( ) > (IS_NPC(ch) ? 100 : LEARNED(ch, gsn_swim )
	+ ch->curr_talent[TAL_WATER] + 10 )))) {
	act(AT_BLUE, "You splash about but are unable to get anywhere.",
		ch, NULL, NULL, TO_CHAR);
	act(AT_BLUE, "$n splashes about ineffectively in the water.", ch,
		NULL, NULL, TO_ROOM);
	learn_from_failure(ch, gsn_swim);
	STRFREE(ch->last_taken);
	ch->last_taken = STRALLOC("splashing about");
	...
	if (number_range(1, get_curr_str(ch)) < 30) {
		send_to_char("You are held in place!\n\r", ch);
		return rNONE;
	} else {
		send_to_char("You break free from the hold!\n\r", ch);
		act(AT_ACTION, "$n breaks free!\n\r", ch, NULL, NULL, TO_ROOM);
		if (IS_NPC(ch))
			xREMOVE_BIT(ch->affected_by, AFF_HOLD);
		else
			xREMOVE_BIT(ch->pcdata->perm_aff, AFF_HOLD);
	}
    }

    if (IS_SET(ch->mood, MOOD_GUARD)) {
        send_to_char("You are guarding this room!\n\r", ch);
        send_to_char("If you want to abandon your post, type GUARD again.\n\r", ch);
        return rNONE;
    }
...
/* slippery floor, chance of falling -keo */
	if (IS_SET(ch->in_room->room_flags, ROOM_SLIPPERY)
	&& !IS_AFFECTED(ch, AFF_FLYING) && !fall && !ch->in_obj) {
		if (number_range(1, 200) >
		get_curr_dex(ch) + ch->curr_talent[TAL_FROST]) {
			send_to_char("You slip and fall!\n\r", ch);
			act(AT_ACTION, "$n slips and falls!", ch, NULL, NULL, TO_ROOM);
			ch->position = POS_RESTING;
			lose_hp(ch, 5);
			STRFREE(ch->last_taken);
			ch->last_taken = STRALLOC("sliding on the ice");
			WAIT_STATE(ch, (200 - get_curr_dex(ch)) / 5);
			return rNONE;
		} else if (ch->curr_talent[TAL_FROST] < 30) {
			send_to_char("You almost slip, but manage to keep your balance.\n\r", ch);
			STRFREE(ch->last_taken);
			ch->last_taken = STRALLOC("sliding on the ice");
			WAIT_STATE(ch, (200 - get_curr_dex(ch)) / 10);
		} else {
			send_to_char("You skate along the ice.\n\r", ch);
		}
	}

/* sticky floor, chance of sticking -keo */
	if (IS_SET(ch->in_room->room_flags, ROOM_STICKY)
	&& !IS_AFFECTED(ch, AFF_FLYING) && !fall) {
		if (number_range(1,200) > get_curr_str(ch)) {
			send_to_char("Your feet stick to the floor.\n\r", ch);
			STRFREE(ch->last_taken);
			ch->last_taken = STRALLOC("stuck to the floor");
			WAIT_STATE(ch, (200 - get_curr_str(ch)) / 5);
			return rNONE;
		}
	}

/* check for guards -keo */
    for ( gch = ch->in_room->first_person; gch; gch = gch->next_in_room )
    {
        if (gch->position > POS_KNEELING 
	&& IS_SET(ch->mood, MOOD_GUARD) && (gch != ch))
        {
		if (IS_SAME_PLANE(ch, gch)) {
	    if(IS_AFFECTED(ch, AFF_FLYING)
	    && !IS_AFFECTED(gch, AFF_FLYING)) {
	act( AT_PLAIN, "$N tries to block you, but you fly right over $S head!", ch, NULL, gch, TO_CHAR);
	act( AT_PLAIN, "You try to block $n, but $e flies right over your head!", ch, NULL, gch, TO_VICT);
	    } else {
		if (gch->weight > ch->weight) {
        act( AT_PLAIN, "$N blocks your exit.", ch, NULL, gch, TO_CHAR );
	act( AT_PLAIN, "You block $n's exit.", ch, NULL, gch, TO_VICT );
            return rNONE;
		} else {
	act( AT_PLAIN, "$N tries to block you but you bowl $M over!", ch,
NULL, gch, TO_CHAR );
	act( AT_PLAIN, "You try to block $n but $e bowls you over!", ch,
NULL, gch, TO_VICT );
	gch->position = POS_RESTING;
	STRFREE(gch->last_taken);
	gch->last_taken = STRALLOC("stunned");
	WAIT_STATE(ch, PULSE_VIOLENCE);
		}
	      }
	   }
...
/* Everything went through, move them! */
	...
	if ( !IS_AFFECTED(ch, AFF_SNEAK)
	|| (!IS_NPC(ch) && number_range(1,20) >
	number_range(1,ch->pcdata->noncombat[SK_STEALTH])))
    {
      if ( fall )
		  if (IS_UNDERWATER(ch))
			  txt = "sinks";
      ...
      {
	if (ch->in_obj && ch->in_obj->action_desc) {
		txt = ch->in_obj->action_desc;
	} else
        if ( ch->mount )
        {
	...
	act( AT_ACTION, buf, ch, NULL, ch->mount, TO_NOTVICT );
        sprintf( buf, "$n %s %s upon your back.", txt, dir_name[door] );
        act( AT_ACTION, buf, ch, NULL, ch->mount, TO_VICT );
      }
      else
      {
	...
	/* Find the trail we made when we came in and tell it where we went -- Scion */

	trail=ch->in_room->first_trail;
	while (trail) {
        /* Expire old trails */
		if (trail->age-current_time+1800 < 0) {
			TRAIL_DATA *tmp_trail;

			tmp_trail=trail->next;
			UNLINK(trail, ch->in_room->first_trail, ch->in_room->last_trail, next, prev);
			STRFREE(trail->name);
			DISPOSE(trail);
			trail=tmp_trail;
			continue;
		} else {
			if (trail->name && !strcmp(ch->name, trail->name)) {
				trail->to=door;
				trail->race=ch->nation;
				if (ch->max_hit/2 > ch->hit)
					trail->blood=TRUE;
				else
				trail->blood=FALSE;
				if (IS_AFFECTED(ch, AFF_FLYING))
	...
	/* Oh so useful to know this -- Scion */
	...
	if (ch->in_obj) {
		...
		obj_from_room(ch->in_obj);
		obj_to_room(ch->in_obj, to_room);
		for (rch = ch->in_room->first_person;rch;rch = rch->next_in_room) 
		{
		    if (rch != ch && rch->in_obj == ch->in_obj) {
			char_from_room(rch);
	...
	/* separately to make sure it displays right */
	if (ch->in_obj) {
	CHAR_DATA *rch;
	...
	/* Create a new trail in the room we moved to unless we already have one -- Scion */
	trail=ch->in_room->first_trail;

	while (trail && trail->name && strcmp(trail->name, ch->name))
		trail=trail->next;

	if (!trail) {
		CREATE(new_trail, TRAIL_DATA, 1);
		new_trail->blood=FALSE;
		new_trail->fly=FALSE;
		new_trail->race=NULL;
		new_trail->to=-1;
		new_trail->from=-1;
		new_trail->age=current_time;
		new_trail->name=NULL;
		LINK(new_trail, ch->in_room->first_trail, ch->in_room->last_trail, next, prev);
		trail=new_trail;
		new_trail->next=NULL;
		new_trail->prev=NULL;
	}
	...
	if (IS_UNDERWATER(ch) && !was_underwater) {
		act(AT_PLAIN, "The water is too deep to walk any further, so you begin to swim.", ch, NULL, NULL, TO_CHAR);
	} else if (!IS_UNDERWATER(ch) && was_underwater) {
		act(AT_PLAIN, "You can touch the bottom here, so you no longer need to swim.", ch, NULL, NULL, TO_CHAR);
	}
    ...
    */
	...
	&& fch->master == ch
	&& fch->position >= POS_SQUATTING
	&& can_see(fch, ch) )
	{
		act( AT_ACTION, "You follow $N.", fch, NULL, ch, TO_CHAR );
		act( AT_ACTION, "$n follows $N.", fch, NULL, ch, TO_NOTVICT);
		act( AT_ACTION, "$n follows you.", fch, NULL, ch, TO_VICT);
		char_from_room(fch);
    ...
    &&  number_range(1,25) > ch->curr_talent[TAL_VOID]) {
	travel_teleport(ch);
	if (number_range(1,10) == 10) {
		act(AT_MAGIC, "You are knocked out of the Void!", ch, NULL, NULL, TO_CHAR);
		act(AT_MAGIC, "$n is knocked into reality!", ch, NULL, NULL, TO_ROOM);
		STRFREE(ch->last_taken);
		ch->last_taken = STRALLOC("seeing stars");
		WAIT_STATE(ch, PULSE_VIOLENCE*3);
	if (IS_NPC(ch))
		xREMOVE_BIT(ch->affected_by, AFF_VOID);
	else
	...
	if (IS_UNDERWATER(ch)) {
	  set_char_color( AT_BLUE, ch);
	  send_to_char(" SPLASH! You land in the water!\n\r", ch);
	} else	
...
void do_open( CHAR_DATA *ch, char *argument )
{
	...
	if (get_curr_int(ch) <= 10) {
		send_to_char("You cannot figure out how to make it work.\r\n", ch);
		return;
	}
...
OBJ_DATA *has_key( CHAR_DATA *ch, int key )
{
	...
	if (ch->curr_talent[TAL_VOID] + ch->curr_talent[TAL_SECURITY] >=95) {
		obj = create_object(get_obj_index(OBJ_VNUM_KEY), 0);
		obj->value[0] = key;
		obj->timer = 1;
		obj_to_char(obj, ch);
		STRFREE(obj->obj_by);
		obj->obj_by = STRALLOC(ch->name);
		act(AT_MAGIC, "You weave the correct key from ether.", ch, NULL, NULL, TO_CHAR);
		act(AT_MAGIC, "$n weaves a key from ether.", ch, NULL, NULL, TO_ROOM);
		return obj;
	}
...
void do_bashdoor( CHAR_DATA *ch, char *argument )
{
	...
	if ( !IS_NPC( ch )
	&& LEARNED(ch, gsn_bashdoor) < 1)
	{
	    send_to_char( "You're not burly enough to bash doors!\n\r", ch );
	    return;
	}
	...
	if ( ( pexit = find_door( ch, arg, FALSE ) ) != NULL )
	{
	    ...
	    &&   ch->move >= 15
	    &&   number_percent( ) < ( chance + get_curr_str( ch ) ))
	    {
	...
	if (can_use_bodypart(ch, BP_RHOOF)
	||  can_use_bodypart(ch, BP_LHOOF)) {
		act(AT_SKILL, "Crash!  You kicked open the $d!", ch, NULL, keyword, TO_CHAR);
		act(AT_SKILL, "$n kicks open the $d!",           ch, NULL, keyword, TO_ROOM);
		STRFREE(ch->last_taken);
		ch->last_taken = STRALLOC("kicking open the door");
		WAIT_STATE(ch, PULSE_VIOLENCE);
	} else {
		act(AT_SKILL, "Crash!  You bashed open the $d!", ch, NULL, keyword, TO_CHAR );
		act(AT_SKILL, "$n bashes open the $d!",          ch, NULL, keyword, TO_ROOM );
		STRFREE(ch->last_taken);
		ch->last_taken = STRALLOC("bashing down the door");
		WAIT_STATE(ch, PULSE_VIOLENCE*2);
		lose_hp(ch, 10);
	}
	...
	if (can_use_bodypart(ch, BP_RHOOF)
	||  can_use_bodypart(ch, BP_LHOOF)) {
		act(AT_SKILL, "WHAAAAM!!! You kick the $d with your hooves, but it doesn't budge.",
			ch, NULL, keyword, TO_CHAR );
		act(AT_SKILL, "WHAAAAM!!! $n kicks the $d, but it holds firm.",
			ch, NULL, keyword, TO_ROOM );
		STRFREE(ch->last_taken);
		ch->last_taken = STRALLOC("kicking the door");
		WAIT_STATE(ch, PULSE_VIOLENCE*2);
	} else {
		...
		act(AT_SKILL, "WHAAAAM!!!  $n bashes against the $d, but it holds strong.",
			ch, NULL, keyword, TO_ROOM );
		STRFREE(ch->last_taken);
		ch->last_taken = STRALLOC("bashing into the door");
		WAIT_STATE(ch, PULSE_VIOLENCE*4);
		lose_hp(ch, 10);
	}
		learn_from_failure(ch, gsn_bashdoor);

		/* Should be able to hear bashing from the other side -- Scion */
		...
		&&    pexit_rev->to_room == ch->in_room )
		{
			...
			sprintf(message, "You hear a loud thump against the door to the %s.", dir_name[pexit_rev->vdir]);
			for ( rch = to_room->first_person; rch; rch = rch->next_in_room ) {
			    act(AT_SKILL, message,
				rch, NULL, pexit_rev->keyword, TO_CHAR );
			}
		}
		}
	...
	else
	{
	    ...
	    lose_hp(ch, 10);
		STRFREE(ch->last_taken);
		ch->last_taken = STRALLOC("bashing into the wall");
		WAIT_STATE(ch, PULSE_VIOLENCE*4);
	    learn_from_failure(ch, gsn_bashdoor);
...
void do_stand( CHAR_DATA *ch, char *argument )
{
	...
	if (!str_cmp(arg, "up" ))
	{
		...
		if (ch->on)
		{
		   for ( paf = ch->on->pIndexData->first_affect; paf; paf = paf->next )
			affect_modify( ch, paf, FALSE );
		   for ( paf = ch->on->first_affect; paf; paf = paf->next )
			affect_modify( ch, paf, FALSE );
		   ch->on = NULL;
		}
                return;
	...
	|| IS_AFFECTED(ch, AFF_DREAMWORLD) )
        ...
        if (IS_SET(ch->in_room->room_flags, ROOM_STICKY)) {
              send_to_char("You wake, but your back is stuck to the floor.\n\r",ch);
	      act(AT_ACTION, "$n wakes up, but can't get up.", ch, NULL, NULL, TO_ROOM);
	      ch->position = POS_RESTING;
              break;
        }
    ...
    case POS_RESTING:
        ...
        if (IS_AFFECTED(ch, AFF_HOLD)) {
                send_to_char("You've fallen and you can't get up.\n\r", ch);
                return;
        }

        if (IS_SET(ch->in_room->room_flags, ROOM_STICKY)) {
           if (number_range(1,100) > get_curr_str(ch)) {
              send_to_char("Your back is stuck to the floor.\n\r",ch);
              return;
           } else {
	      send_to_char("You pry your back off the floor.\n\r",ch);
           }
        }
    ...
    case POS_KNEELING:
        ...
        if (IS_SET(ch->in_room->room_flags, ROOM_STICKY)) {
           if (number_range(1,100) > get_curr_str(ch)) {
              send_to_char("Your knees are stuck to the floor.\n\r",ch);
              return;
           } else {
              send_to_char("You pry your knees off the floor.\n\r",ch);
           }
        }

	send_to_char( "You get off your knees and stand.\n\r", ch);
	act(AT_ACTION, "$n clambers to $s feet.", ch, NULL, NULL, TO_ROOM);
	ch->position = POS_STANDING;
	break;
    ...
    case POS_SQUATTING:
        send_to_char( "You stand up straight.\n\r", ch );
        act( AT_ACTION, "$n stands up straight.", ch, NULL, NULL, TO_ROOM);
        ch->position = POS_STANDING;
        break;
...
void do_sit( CHAR_DATA *ch, char *argument )
{
    ...
    case POS_SQUATTING:
    case POS_KNEELING:
	send_to_char( "You cross your legs and sit.\n\r", ch);
	act(AT_ACTION, "$n crosses $s legs and sits.", ch, NULL, NULL, TO_ROOM);
	ch->position = POS_SITTING;
	break;
	...
	if(arg[0])
	{
		...
		if (((type == SI_ON) && (!IS_SET(obj->value[2], SIT_ON)))
			|| ((type == SI_AT) && (!IS_SET(obj->value[2], SIT_AT)))
			|| ((type == SI_IN) && (!IS_SET(obj->value[2], SIT_IN)))
			|| ((ch->on != obj) && (count_users(obj) >= obj->value[0])))
		{
			sprintf (buf, "There is no place to sit %s %s.\n\r",
				type_word, obj->short_descr);
			send_to_char (buf, ch );
			return;
		}
        	ch->position = POS_SITTING;
		/* Heavy characters ought to break furniture! -- Scion :P */
		if (!xIS_SET(obj->extra_flags, ITEM_MAGIC) && 
		ch->weight >= ((obj->value[0] * 300) + number_range(-30, 30))) {
			act( AT_ACTION, "$p breaks under your weight!", ch, obj, NULL, TO_CHAR );
			act( AT_ACTION, "$p breaks under $n's weight!", ch, obj, NULL, TO_ROOM );
			make_scraps( obj );
			ch->position = POS_RESTING;
			break;
		} else {
			...
			for ( paf = ch->on->first_affect; paf; paf = paf->next )
				affect_modify( ch, paf, TRUE );
			sprintf (buf, "You sit %s $p.", type_word );
			act (AT_ACTION,buf, ch, obj, NULL,TO_CHAR );
			sprintf (buf, "$n sits %s $p.", type_word );
			act (AT_ACTION,buf, ch, obj, NULL, TO_ROOM );
			break;
	 		}
		}
...
/* will need to add more furniture flags if we want people to be
 * able to kneel on stuff -keo
 */
void do_kneel( CHAR_DATA *ch, char *argument )
{
    ...
    case POS_SLEEPING:
        ...
        send_to_char( "You wake and climb to your knees.\n\r", ch );
        act( AT_ACTION, "$n wakes and climbs to $s knees.", ch, NULL, NULL, TO_ROOM );
        ch->position = POS_KNEELING;
        break;
    case POS_RESTING:
    case POS_SITTING:
	send_to_char("You climb to your knees.\n\r", ch);
	act(AT_ACTION, "$n climbs to $s knees.", ch, NULL, NULL, TO_ROOM);
	ch->position = POS_KNEELING;
	break;
    case POS_SQUATTING:
    case POS_STANDING:
	send_to_char("You drop to your knees.\n\r", ch);
	act(AT_ACTION, "$n drops to $s knees.", ch, NULL, NULL, TO_ROOM);
	ch->position = POS_KNEELING;
	break;
    case POS_MOUNTED:
	send_to_char("I don't think your mount would like that.\n\r", ch);
    case POS_KNEELING:
...
/* will need to add more furniture flags if we want people to be
 * able to squat on stuff -keo
 */
void do_squat( CHAR_DATA *ch, char *argument )
{
    switch ( ch->position ) {
    case POS_SLEEPING:
        if ( IS_AFFECTED(ch, AFF_SLEEP) )
        ...
        send_to_char( "You wake and crouch.\n\r", ch );
        act( AT_ACTION, "$n wakes and crouches.", ch, NULL, NULL, TO_ROOM);
        ch->position = POS_SQUATTING;
        break;
    case POS_RESTING:
    case POS_SITTING:
    case POS_KNEELING:
	send_to_char("You move into a crouch.\n\r", ch);
	act(AT_ACTION, "$n moves into a crouch.", ch, NULL, NULL, TO_ROOM);
	ch->position = POS_SQUATTING;
	break;
    case POS_STANDING:
	send_to_char("You crouch down.\n\r", ch);
	act(AT_ACTION, "$n crouches down.", ch, NULL, NULL, TO_ROOM);
	ch->position = POS_SQUATTING;
	break;
    case POS_MOUNTED:
	send_to_char("I don't think your mount would like that.\n\r", ch);
    case POS_SQUATTING:
	send_to_char("You are already crouching.\n\r", ch);
    }
}
...
void do_rest( CHAR_DATA *ch, char *argument )
{
    ...
    case POS_SQUATTING:
    case POS_KNEELING:
	send_to_char( "You lie down and rest.\n\r", ch);
	act(AT_ACTION, "$n lies down and rests.\n\r", ch, NULL, NULL, TO_ROOM);
	ch->position = POS_RESTING;
	break;
	...
	if(arg[0])
	{
		...
		act (AT_ACTION,buf, ch, obj, NULL,TO_CHAR );
		sprintf (buf, "$n rests %s $p.", type_word );
		act (AT_ACTION,buf, ch, obj, NULL, TO_ROOM );
		break;
		}
	}
...
void do_sleep( CHAR_DATA *ch, char *argument )
{
	...
	if(arg[0])
	{
		...
		act (AT_ACTION,buf, ch, obj, NULL,TO_CHAR );
		sprintf (buf, "$n sleeps %s $p.", type_word );
		act (AT_ACTION,buf, ch, obj, NULL, TO_ROOM );
		ch->position = POS_SLEEPING;
		break;
    	}
...
void do_wake( CHAR_DATA *ch, char *argument )
{
    ...
    { act( AT_PLAIN, "You can't seem to wake $M!",  ch, NULL, victim, TO_CHAR );  return; }
        ...
        if (IS_SET(ch->in_room->room_flags, ROOM_STICKY)) {
           act(AT_ACTION, "You wake $M, but $s back is stuck to the floor.\n\r",ch, NULL, victim, TO_CHAR);
	   victim->position = POS_RESTING;
	   act(AT_ACTION, "$n wakes you, but your back is stuck to the floor.\n\r", ch, NULL, victim, TO_VICT);
           return;
        }
...
void do_enter( CHAR_DATA *ch, char *argument )
{
	...
	/* If we got this far, we must have an obj */
	if (!obj 
	...
	act(AT_ACTION, "$n gets in $p.", ch, obj, NULL, TO_ROOM);
	act(AT_ACTION, "You get inside $p.", ch, obj, NULL, TO_CHAR);
	ch->in_obj = obj;
...
void do_leave( CHAR_DATA *ch, char *argument )
{
	...
	if (ch->in_obj) {
		obj = ch->in_obj;
		act(AT_ACTION, "$n gets out of $p.",
			ch, ch->in_obj, NULL, TO_ROOM);
		act(AT_ACTION, "You get out of $p.",
			ch, ch->in_obj, NULL, TO_CHAR);
		ch->in_obj = NULL;
		return;
	}
	for ( pexit = ch->in_room->first_exit; pexit; pexit = pexit->next )
...
ch_ret pullcheck( CHAR_DATA *ch, int pulse )
{
	...
	/* it wasn't so appropriate, moved below -keo */
/*	if ( destrm && xit->to_room->first_person )
	...
	for ( obj = room->first_content; obj; obj = obj_next )
	{
	    ...
	    if ( IS_OBJ_STAT(obj, ITEM_BURIED) || IS_OBJ_STAT(obj, ITEM_NO_TAKE) )
		continue;
...
/* Sets a character's home room -- Scion */
void do_home(CHAR_DATA *ch, char *argument) {
	...
	if (IS_NPC(ch)) {
		send_to_char("You haven't such a great memory.\n\r", ch);
		return;
	}
	...
	if (!strcmp(arg, "list")) {
		send_to_char("Memorized locations:\r\n",ch);
		for (slot=0; slot<8; slot++) {
			room=get_room_index(ch->pcdata->memorize[slot]);
			sprintf(arg, "%d: %s\r\n", slot, (room ? room->name : "None"));
			send_to_char(arg, ch);
		}
		send_to_char("You will recall to slot 0.\r\n",ch);
		return;
	} 

/* Fix later */	
/*	if () {
		send_to_char("You cannot see your surroundings to memorize them.\n\r", ch);
		return;
	}
*/
	switch (ch->substate) {
	...
	case SUB_NONE:
		add_timer( ch, TIMER_DO_FUN, 5, do_home, 1);
		send_to_char( "You begin to memorize your surroundings...\n\r", ch );
		act( AT_PLAIN, "$n looks around, trying to memorize $s surroundings...", ch, NULL, NULL, TO_ROOM);
		ch->recall=slot;
		return;

	case SUB_TIMER_DO_ABORT:
		act( AT_PLAIN, "$n cannot seem to concentrate.",ch, NULL, NULL, TO_ROOM);
		send_to_char("You just can't seem to concentrate...\r\n",ch);
		ch->substate=SUB_NONE;
		return;
	
	default:
		ch->substate = SUB_NONE;
		ch->pcdata->memorize[ch->recall]=ch->in_room->vnum;
		act( AT_PLAIN, "$n nods, indicating that $e has memorized $s surroundings.", ch, NULL, NULL, TO_ROOM);
		send_to_char( "You now feel familiar with this place.\r\n",ch);
		return;
	}
}

/* Moved here from the old clans.c, what the hell where they doing there anyway? -- Scion */
...
chance += ((get_curr_str(ch) + ch->weight) - ((get_curr_str(victim) + get_curr_dex(victim)) / 2) + victim->weight);
...
if (chance < number_percent( ))
{
  ...
  return;
	...
	act( AT_ACTION, "$n shoves $N.", ch, NULL, victim, TO_NOTVICT );
	...
	STRFREE(ch->last_taken);
	ch->last_taken = STRALLOC("shoving");
	WAIT_STATE(ch, PULSE_VIOLENCE);
	STRFREE(victim->last_taken);
	victim->last_taken = STRALLOC("being shoved");
	WAIT_STATE(ch, PULSE_VIOLENCE);
}
...
void do_drag( CHAR_DATA *ch, char *argument )
{
    ...
    if ( ( victim = get_char_room( ch, arg ) ) == NULL )
    {
		...
		if (obj && obj->carried_by) {
			send_to_char( "Objects must be on the ground in order to be dragged.\r\n", ch);
			return;
		}
    }

	/* The desired thing to drag is an obj, not a char -- Scion */
	if (!victim) {
		if (ch->in_room->pShop) {
			send_to_char("You can't drag items without paying in a shop.\n\r", ch);
			return;
		}
		if ( arg2[0] == '\0' )
		{
			send_to_char( "Drag it in which direction?\n\r", ch);
			return;
		}
	    ...
	    to_room = pexit->to_room;
		chance += ((ch->weight + get_curr_str(ch)/4) - (obj->weight));

		move_drain = number_percent() - chance;

		if (ch->move <= 0 || chance < move_drain+chance) {
			send_to_char("You can't seem to move it.\n\r", ch);
			return;
		}

		act( AT_ACTION, "You drag $p into the next room.", ch, obj, NULL, TO_CHAR );
		act( AT_ACTION, "$n drags $p from the room.", ch, obj, NULL, TO_ROOM);
		obj_from_room(obj);
		/* Move ch to the room too.. they are doing dragging - Scryn */
		move_char( ch, get_exit(ch->in_room,exit_dir), 0);
		obj_to_room(obj, to_room);
		act(AT_ACTION, "$n drags $p in behind $m.", ch, obj, NULL, TO_ROOM);
		for (victim=ch->was_in_room->first_person; victim; victim=victim->next_in_room) {
			if (victim->on==obj) {
		...
		ch->move -= URANGE(5, move_drain, 100);
		return;
	}
    ...
    if ( /*!IS_SET(victim->pcdata->flags, PCFLAG_DEADLY) && -- Scion */ victim->position > 8 )
    {
...
chance += ((ch->weight + get_curr_str(ch)/10) - (victim->weight));
...
{
  send_to_char("You failed.\n\r", ch);
  act( AT_ACTION, "$n tries to drag you from the room, but fails.", ch, NULL, victim, TO_VICT);
  return;
}
    if ( victim->position < POS_STANDING )
    {
	...
	act( AT_ACTION, "$n grabs you and drags you from the room.", ch, NULL, victim, TO_VICT ); 
	move_char( victim, get_exit(ch->in_room,exit_dir), 0);
	if ( !char_died(victim) )
...
/* Move ch to the room too.. they are doing dragging - Scryn */
	...
	ch->move -= URANGE(5, move_drain, 100);
	STRFREE(ch->last_taken);
	ch->last_taken = STRALLOC("dragging");
	WAIT_STATE(ch, PULSE_VIOLENCE);
	STRFREE(victim->last_taken);
	victim->last_taken = STRALLOC("being dragged");
	WAIT_STATE(ch, PULSE_VIOLENCE);
	return;
    }
...
/* Typing 'fly' should make you perm fly until you run out of moves or type 'land' -- Scion */
void do_fly(CHAR_DATA *ch, char *argument) {
	if (!can_use_bodypart(ch, BP_RWING)
	&&  !can_use_bodypart(ch, BP_LWING)) {
		send_to_char("You can't seem to be able to get off the ground.\r\n", ch);
		return;
	}
	...
	act(AT_PLAIN, "You spread your wings and take to the sky.",
		ch, NULL, NULL, TO_CHAR);
	act(AT_PLAIN, "$n spreads $s wings and takes to the sky.",
		ch, NULL, NULL, TO_ROOM);
	if (IS_NPC(ch))
	...
	STRFREE(ch->last_taken);
	ch->last_taken = STRALLOC("taking off");
	WAIT_STATE(ch, PULSE_VIOLENCE);
}

/* Stop flying (gracefully, we hope) -- Scion */
void do_land(CHAR_DATA *ch, char *argument) {
	if (!has_bodypart(ch, BP_RWING)) {
		send_to_char("How exactly do you intend to do that?\r\n", ch);
		return;
	}

	if (!IS_AFFECTED(ch, AFF_FLYING)) {
		send_to_char("You aren't flying at the moment.\r\n", ch);
		return;
	}

	act(AT_PLAIN, "You settle gracefully to the ground.", ch, NULL, NULL, TO_CHAR);
	act(AT_PLAIN, "$n settles gracefully to the ground.", ch, NULL, NULL, TO_ROOM);
	if (IS_NPC(ch))
		xREMOVE_BIT(ch->affected_by, AFF_FLYING);
	else
		xREMOVE_BIT(ch->pcdata->perm_aff, AFF_FLYING);
	STRFREE(ch->last_taken);
	ch->last_taken = STRALLOC("landing");
	WAIT_STATE(ch, PULSE_VIOLENCE);
}


/* get rid of all their dream items. modified from drop_artifacts -keo */
void drop_dream_items(CHAR_DATA *ch, OBJ_DATA *obj) {
        ...
        while (obj) {
                if (obj->last_content)
                        drop_dream_items(ch, obj->last_content);
