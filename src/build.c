...
char *	const	v_flags [] =
{ "land", "surface", "underwater", "air", "space", "underground", "climb", 
"airtight", "sand", "ice" };
...
"spar", "vampire", "rw2",
"counted", "r23", "r24", "r25", "r26", "r27", "r28", "r29", "r30", "r31"
};
...
void do_matshop( CHAR_DATA *ch, char *argument ) {
	...
	if (!strcmp(arg1, "load")) {
		...
		matshop->mat[1], matshop->mat[2], matshop->mat[3],
		matshop->mat[4], matshop->mat[0]);
		send_to_char(buf, ch);
                return;
        }
...
void do_material( CHAR_DATA *ch, char *argument ) {
	...
	if (!strcmp(arg1, "list")) { /* Let's go for the easy one first :P */
		if (!first_material) {
	...
	if (!strcmp(arg1, "create")) {
		...
		if (!first_material) { /* Ok, get this new material added into the list */
			...
			material=first_material;
			while (material->next) {
				if (material->number != material->next->number-1) { /* and keep the numbers sequential */
...
void do_ingredient( CHAR_DATA *ch, char *argument ) {
	...
	OBJ_INDEX_DATA *pObjIndex;
        ...
        if (!strcmp(arg1, "list")) {
                if (!first_ingred) {
                        send_to_char("No alchemical ingredientss have been defined.\r\n",ch);
                        return;
                }
                ingred = first_ingred;
...
void do_recipe( CHAR_DATA *ch, char *argument ) {
	...
	argument = one_argument(argument, arg4);
		...
		pObjIndex->short_descr : "???",
                (pObjIndex = get_obj_index(recipe->result)) ?
		pObjIndex->short_descr : "???");
                        send_to_char(buf, ch);
                        recipe=recipe->next;
                    }
	...
	if (!strcmp(arg1, "find")) {
		int i = atoi(arg2);
                ...
                if (arg3 && !str_cmp(arg3, "name")) {
                    while (recipe) {
                    if (recipe->result == i
                    || recipe->ingred1 == i || recipe->ingred2 == i) {
                        sprintf(buf, "%d: %s + %s = %s\r\n",
                                recipe->number,
                (pObjIndex = get_obj_index(recipe->ingred1)) ?
                pObjIndex->short_descr : "???",
...
void do_moon( CHAR_DATA *ch, char *argument ) {
	...
	if (ch->curr_talent[TAL_VOID] < 102) {
...
void do_species( CHAR_DATA *ch, char *argument ) {
    ...
    if ( !str_cmp( arg3, "part" )) {
        ...
        if (!strcmp(arg1, "save")) {
                ...
                for (species=first_species; species;
species=species->next) {
                        fprintf(fp, "\n#SPECIES\n");
                        fprintf(fp, "Name       %s~\n", species->name);
	...
	if (!argument || argument[0] == '\0') {
		...
		ch_printf(ch, "&cThere are &C%d &craces in existance.\n\r", i);
		    ...
		    if (!str_cmp(nation->species, argument)
			  ...
			  ||   IS_SET(ch->pcdata->permissions, PERMIT_MISC))) {
                        sprintf(buf, "&G%s%s\r\n", nation->name,
				IS_SET(nation->flags, NAT_NPC) ? " &z(hidden)" : "");
                        send_to_char(buf, ch);
                    }
                    nation = nation->next;
                }
...
*/	
	fprintf( fpout, "%d %d %d %d %d %d %d %d\n", 
		...
		room->water, room->curr_water, 
		room->elevation, room->curr_elevation,
		room->resources, room->curr_resources);
...
void do_rlist( CHAR_DATA *ch, char *argument )
{
    ...
    if ( IS_NPC(ch) || !ch->pcdata
    || ( !ch->pcdata->area && !IS_SET(ch->pcdata->permissions, PERMIT_HIBUILD)) )
    {
...
void do_climate(CHAR_DATA *ch, char *argument)
{
	...
	if(arg[0] == '\0')
	{
		...
		ch_printf(ch, "\tMana:           %s\n\r",
			mana_settings[area->weather->climate_mana]);
