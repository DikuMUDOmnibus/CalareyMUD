...
void load_mobiles( AREA_DATA *tarea, FILE *fp )
{
	...
	/* was align */			  fread_number( fp );
	letter				= fread_letter( fp );
	/* was level */			  fread_number( fp );
	...
	else
	{
	    ...
	    pMobIndex->perm_wis		= 100;
	    pMobIndex->perm_cha		= 100;
...
void load_ranges( AREA_DATA *tarea, FILE *fp )
{
	...
	tarea->low_soft_range = tarea->prev ?
		tarea->prev->low_soft_range+1 : 0;
...
CHAR_DATA *create_mobile( MOB_INDEX_DATA *pMobIndex )
{
	...
	if (mob->height == 0) mob->height = 500;
	if (mob->weight == 0) mob->weight = 2000;
    ...
    if ( !pMobIndex->hitnodice )
		mob->max_hit = UMAX(get_curr_con(mob)
		* 30, 100);
    else
		mob->max_hit = pMobIndex->hitnodice *
...
void do_memory( CHAR_DATA *ch, char *argument )
{
    ...
    ch_printf_color( ch, "&gIdxMobs: &G%5d     &gMobiles: %s%5d\n\r",
        top_mob_index, (MAX_MOBS*top_area >= nummobsloaded ? "&G" : "&R"), 
	nummobsloaded );
    ch_printf_color( ch, "&gIdxObjs: &G%5d     &gObjs:    %s%5d(%d)\n\r",
        top_obj_index, (MAX_OBJS*top_area >= numobjsloaded ? "&G" : "&R"), 
...
char *aoran( const char *str )
{
	...
	if ( isavowel(buf[0])
    || ( strlen(buf) > 1 && LOWER(buf[0]) == 'y' && !isavowel(buf[1])) )
      strcpy( temp, "an " );
    else
      strcpy( temp, "a " );
    strcat( temp, str );
...
void load_nations( void ) {
	...
	if (nation->ac_plus < 0) nation->ac_plus = abs(nation->ac_plus / 10);
