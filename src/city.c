...
void do_city(CHAR_DATA *ch, char *argument) {
	...
	if (!str_cmp(arg1, "list")) {
		...
		city->peasants, city->builders, city->farmers,
		city->merchants, city->miners);
		ch_printf(ch, "Houses: %d  Farms: %d  Shops: %d  Forests: %d  Mines: %d\n\r",
		city->houses, city->farms, city->shops, city->forests,
		city->mines);
                return;
        }
        if (!strcmp(arg1, "set")) {
	...
	if (!str_cmp(arg1, "house")) {
		...
		SET_BIT(ch->in_room->room_flags, ROOM_INDOORS);
                ...
                switch(number_range(1,5)) {
                        case 1: sprintf(buf, "quaint "); break;
                        case 2: sprintf(buf, "small "); break;
                        case 3: sprintf(buf, "cozy "); break;
                        case 4: sprintf(buf, "filthy "); break;
                        case 5: sprintf(buf, "comfortable "); break;
                }
                switch(number_range(1,5)) {
                        case 1: strcat(buf, "house"); break;
                        case 2: strcat(buf, "home"); break;
                        case 3: strcat(buf, "cottage"); break;
                        case 4: strcat(buf, "hovel"); break;
                        case 5: strcat(buf, "shack"); break;
                }
		...
		SET_BIT(ch->in_room->room_flags, ROOM_INDOORS);
                STRFREE(ch->in_room->name);
                switch(number_range(1,5)) {
                        case 1: sprintf(buf, "well-tended "); break;
                        case 2: sprintf(buf, "well-stocked "); break;
                        case 3: sprintf(buf, "clean "); break;
                        case 4: sprintf(buf, "general goods "); break;
                        case 5: sprintf(buf, "clothing "); break;
                }
                switch(number_range(1,5)) {
                        case 1: strcat(buf, "shop"); break;
                        case 2: strcat(buf, "store"); break;
                        case 3: strcat(buf, "merchant"); break;
                        case 4: strcat(buf, "stall"); break;
                        case 5: strcat(buf, "market"); break;
                }
                ch->in_room->name = STRALLOC(aoran(buf));
		...
		SET_BIT(ch->in_room->room_flags, ROOM_INDOORS);
                STRFREE(ch->in_room->name);
                switch(number_range(1,5)) {
                        case 1: sprintf(buf, "dusty "); break;
                        case 2: sprintf(buf, "dark "); break;
                        case 3: sprintf(buf, "dank "); break;
                        case 4: sprintf(buf, "musty "); break;
                        case 5: sprintf(buf, "dirty "); break;
                }
                switch(number_range(1,5)) {
                        case 1: strcat(buf, "mine"); break;
                        case 2: strcat(buf, "mining tunnel"); break;
                        case 3: strcat(buf, "mining cave"); break;
                        case 4: strcat(buf, "mining cavern"); break;
                        case 5: strcat(buf, "mineshaft"); break;
                }
                ch->in_room->name = STRALLOC(aoran(buf));
		...
		ch->in_room->sector_type = SECT_CITY;
                STRFREE(ch->in_room->name);
                switch(number_range(1,5)) {
                        case 1: sprintf(buf, "cobblestone "); break;
                        case 2: sprintf(buf, "gravel "); break;
                        case 3: sprintf(buf, "dirt "); break;
                        case 4: sprintf(buf, "paved "); break;
                        case 5: sprintf(buf, "well-trod "); break;
                }
                switch(number_range(1,5)) {
                        case 1: strcat(buf, "road"); break;
                        case 2: strcat(buf, "lane"); break;
                        case 3: strcat(buf, "boulevard"); break;
                        case 4: strcat(buf, "avenue"); break;
                        case 5: strcat(buf, "street"); break;
                }
                ch->in_room->name = STRALLOC(aoran(buf));
	} else {
...
void do_plant(CHAR_DATA *ch, char *argument) {
	...
	if (!str_cmp(arg1, "forest")) {
		...
		city->forests++;
	        STRFREE(ch->in_room->name);
		switch(number_range(1,5)) {
			case 1: sprintf(buf, "pine "); break;
			case 2: sprintf(buf, "drenak "); break;
			case 3: sprintf(buf, "lern "); break;
			case 4: sprintf(buf, "cherry "); break;
			case 5: sprintf(buf, "maple "); break;
		}
		switch(number_range(1,5)) {
			case 1: strcat(buf, "forest"); break;
			case 2: strcat(buf, "grove"); break;
			case 3: strcat(buf, "glade"); break;
			case 4: strcat(buf, "orchard"); break;
			case 5: strcat(buf, "stand"); break;
		}
	        ch->in_room->name = STRALLOC(aoran(buf));
		...
		city->farms++;
                STRFREE(ch->in_room->name);
                switch(number_range(1,5)) {
                        case 1: sprintf(buf, "zim'adar "); break;
                        case 2: sprintf(buf, "calluv "); break;
                        case 3: sprintf(buf, "corn "); break;
                        case 4: sprintf(buf, "jazma "); break;
                        case 5: sprintf(buf, "worfberry "); break;
                }
                switch(number_range(1,2)) {
                        case 1: strcat(buf, "farm"); break;
                        case 2: strcat(buf, "field"); break;
                }
                ch->in_room->name = STRALLOC(aoran(buf));
	} else {
