...
char * const quest_flag[] =
{ "eternal_night", "tempest", "evil", "r4", "r5", "r6", "r7", "r8", "r9",
...
void do_otransfer(CHAR_DATA *ch, char *argument) {
	...
	if (obj->carried_by) {
		act(AT_YELLOW, "$p vanishes from your hands.",
...
void do_wizhelp( CHAR_DATA *ch, char *argument ) {
	...
	if (!IS_SET(ch->pcdata->permissions, 1 << permit)) continue;
...
void do_rstat( CHAR_DATA *ch, char *argument )
{
	...
	case SECT_ICE:			sect = "Ice";		break;
	case SECT_BEACH:		sect = "Beach";		break;
    }
	...
	flag_string(location->runes, rune_flags) );
...
/* lets see if we can make life easier , show the progtypes - shogar */
	...
	ext_flag_string(&location->progtypes,mprog_flags));
...
/* lets see if we can make life easier , show the progtypes - shogar */
    ch_printf_color(ch,"&cProgs: &w%s\n\r",
	ext_flag_string(&obj->pIndexData->progtypes,mprog_flags));
...
void do_mstat( CHAR_DATA *ch, char *argument )
{
	...
	pager_printf_color( ch, "&cStr: &C%2d&c )( Int: &C%2d&c )( Wil: &C%2d&c )( Dex: &C%2d&c )( Con: &C%2d&c )( Per: &C%2d&c )( End: &C%2d&c\n\r",
	get_curr_str(victim),
	get_curr_int(victim),
	get_curr_wis(victim),
	get_curr_dex(victim),
	...
	if (!IS_NPC(victim) && (victim->nation)) {
	  pager_printf_color( ch,
	  "&cNation  : &P%-3s       ", victim->nation->name );
	}
	if (!IS_NPC(victim) && (victim->pcdata->parent)) {
	   pager_printf_color( ch,
	   "&cParent : &C%s          ", victim->pcdata->parent );
	}
    if ( victim->race  < MAX_NPC_RACE  && victim->race  >= 0 )
      pager_printf_color( ch, "&cRace      : &w%-2.2d/%-10s\n\r",
	  victim->race,
	...
	GET_HITROLL(victim), GET_AC(victim) );
	...
	victim->pcdata->condition[COND_DRUNK] );
      ...
      send_to_pager( "\n\r", ch );
    pager_printf_color( ch, "&cSave versus: &w%d %d %d %d %d       &cItems: &w(%d/%d)  &cWeight carried &w(%d/%d) &cTotal weight &w(%d)\n\r",
    ...
    if (!IS_NPC(victim))
	    ...
	    affect_bit_name( &victim->pcdata->perm_aff));
    if ( IS_NPC( victim ) )
      pager_printf_color( ch, "&cShortdesc  : &w%s\n\r",
	victim->short_descr != NULL ? victim->short_descr : "(none set)" );
    else
    {
...
/* lets see if we can make life easier , show the progtypes - shogar */
    if(IS_NPC(victim))
    {
		...
		ext_flag_string(&victim->pIndexData->progtypes,mprog_flags));
    }
...
void do_gwhere( CHAR_DATA *ch, char *argument )
{
    ...
    pager_printf_color( ch, "\n\r&cGlobal player locations:&w\n\r");
	    ...
	    get_char_worth(victim), victim->name, victim->in_room->vnum, victim->in_room->area->name, victim->in_room->name );
	    count++;
        }
...
void do_oinvoke( CHAR_DATA *ch, char *argument )
{
    ...
    if (!material) material = first_material;
	...
	obj->short_descr, pObjIndex->vnum, pObjIndex->name );
    return;
}
...
void do_balefire( CHAR_DATA *ch, char *argument ) {
    ...
    +   ch->curr_talent[TAL_VOID] < 200) {
        huh(ch);
        return;
    }

        act(AT_DGREY, "A bar of balefire beams from your $t!",
                ch, hands(ch), NULL, TO_CHAR);
        act(AT_DGREY, "A bar of balefire beams from $n's $t!",
	...
	+ TALENT(ch, TAL_TIME))*2,
        MAG_ANTIMATTER, TAL_TIME, FALSE);
}
        ...
        if ( !IS_SET(ch->pcdata->permissions, PERMIT_HIBUILD) )
        {
 	    ...
 	    return;
          ...
          else
          {
            ...
            if ( current_time - ch->pcdata->restore_time <
		RESTORE_INTERVAL * 100 / TALENT(ch, TAL_HEALING) )
            {
...
void do_silence( CHAR_DATA *ch, char *argument )
{
    ...
    if (get_curr_wil(victim) >
	number_range(1, get_curr_wil(ch) + TALENT(ch, TAL_SPEECH))) {
		act(AT_MAGIC, "$N resists your attempt to silence $M.",
			ch, NULL, victim, TO_CHAR);
		act(AT_MAGIC, "You resist $n's attempt to silence you.",
...
void do_destroy( CHAR_DATA *ch, char *argument )
{
  ...
  if ( !victim ) {
    ...
    act(AT_PLAIN, "You are wracked with uncontrollable pain as you are extracted from the matrix.", victim, NULL, NULL, TO_CHAR);
    while ( (obj = victim->last_carrying) != NULL )
        extract_obj(obj);
...
void do_sedit( CHAR_DATA *ch, char *argument )
{
	...
	ch_printf( ch, "Social: %s\n\r\n\rFlags: %s\n\r",
	    social->name,	flag_string(social->flags, soc_flags) );
      ch_printf( ch, "CNoArg: %s\n\r", 
	    social->char_no_arg, ch);
...
void do_showweather(CHAR_DATA *ch, char *argument)
{
	...
	ch_printf(ch, "%-40s%-8s %-8s %-8s %-8s\n\r",
		"Area Name:", "Temp:", "Precip:", "Wind:", "Mana:");
...
// This one.. is just funny =p   -Triskal
    ...
    else if ( !str_cmp( arg2, "demon" ) )
    {
      ...
      act( AT_BLOOD, "foul creature turns on $N, who screams in panic before being eaten alive.",  ch, NULL, victim, TO_CHAR );
      act( AT_BLOOD, "$n gestures, and a slavering demon appears.  The foul creature turns on",  ch, NULL, victim, TO_VICT );
      act( AT_BLOOD, "you with a horrible grin.   You scream in panic before being eaten alive.",  ch, NULL, victim, TO_VICT );
      act( AT_BLOOD, "$n gestures, and a slavering demon appears.  With a horrible grin, the",  ch, NULL, victim, TO_NOTVICT );
      act( AT_BLOOD, "foul creature turns on $N, who screams in panic before being eaten alive.",  ch, NULL, victim, TO_NOTVICT );
    }
