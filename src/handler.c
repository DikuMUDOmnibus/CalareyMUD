...
void huh(CHAR_DATA *ch) {
	...
	switch (number_range(1,6)) {
	    ...
	    case 2:
		ch_printf(ch, "What%s?\n\r",
		(number_range(1,2) == 1 ? " was that again" : ""));
		break;
	    case 3:
		if (ch->sex == 2)
		...
		(number_range(1,2) == 1 ? " here" : ""));
		break;
	    case 5:
		ch_printf(ch, "Sorry, I didn't%s catch that.\n\r",
		(number_range(1,2) == 1 ? " quite" : ""));
		break;
	}
}
...
int moon_visible(AREA_DATA *tarea, int flag) {
	...
	if (!str_cmp(tarea->resetmsg, "Void")) return 0; 
        moon=first_moon;
        while (moon) {
		if (moon->world == NULL || moon->world[0] == '\0') continue;
                if (moon->type == flag && moon->up &&
		!str_cmp(moon->world, tarea->resetmsg))
                       amt += moon->phase;
                moon=moon->next;
        }	
	return amt;
...
bool is_bane(OBJ_DATA *weapon, CHAR_DATA *victim) {
	...
	if (!xIS_SET(weapon->extra_flags, ITEM_MAGIC)) return FALSE;
	if (weapon->value[0]) {
		...
		else switch (victim->race) {
			case 0: weapon->value[0] = BANE_HUMAN; break;
			case 1: weapon->value[0] = BANE_ELF; break;
			case 2: weapon->value[0] = BANE_DWARF; break;
			case 4: weapon->value[0] = BANE_FAERIE; break;
			case 6: weapon->value[0] = BANE_GIANT; break;
			case 7: weapon->value[0] = BANE_ORC; break;
			case 8: weapon->value[0] = BANE_TROLL; break;
			case 15: weapon->value[0] = BANE_MINOTAUR; break;
			case 18: weapon->value[0] = BANE_HALFLING; break;
			case 20: weapon->value[0] = BANE_CENTAUR; break;
			case 35: weapon->value[0] = BANE_DRAGON; break;
		}
	}
	return FALSE;
}
...
void learn_weapon(CHAR_DATA *ch, int i) {
        if (!IS_NPC(ch) && ch->pcdata->weapon[i] < 100) {
            ...
            if (ch->pcdata->weapon_exp[i] +
		exp_level(ch, ch->pcdata->weapon[i])
                >= exp_level(ch, ch->pcdata->weapon[i]+1)) {
...
void learn_noncombat(CHAR_DATA *ch, int i) {
		...
		exp_level(ch, ch->pcdata->noncombat[i])
                >= exp_level(ch, ch->pcdata->noncombat[i]+1)) {
...
sh_int get_curr_str( CHAR_DATA *ch )
{
	...
	if (ch->nation) stat += ch->nation->str_mod;
...
sh_int get_curr_int( CHAR_DATA *ch )
{
    sh_int stat;
        ...
        if (ch->nation) stat += ch->nation->int_mod;
...
sh_int get_curr_wis( CHAR_DATA *ch )
{
    sh_int stat;
        ...
        stat = ch->perm_wis + ch->mod_wis;
        if (ch->nation) stat += ch->nation->wis_mod;
...
sh_int get_curr_dex( CHAR_DATA *ch )
{
    sh_int stat;
        ...
        if (ch->nation) stat += ch->nation->dex_mod;
...
sh_int get_curr_con( CHAR_DATA *ch )
{
    sh_int stat;
        ...
        if (ch->nation) stat += ch->nation->con_mod;
...
sh_int get_curr_cha( CHAR_DATA *ch )
{
    sh_int stat;
        ...
        stat = ch->perm_cha + ch->mod_cha;
        if (ch->nation) stat += ch->nation->cha_mod;
...
sh_int get_curr_end( CHAR_DATA *ch )
{
    sh_int stat;
        ...
        if (ch->nation) stat += ch->nation->lck_mod;
...
int can_carry_w( CHAR_DATA *ch )
{
    ...
    return get_curr_str(ch) * 10 + ch->weight
	 + TALENT(ch, TAL_MOTION) * 10;
}
...
void affect_modify( CHAR_DATA *ch, AFFECT_DATA *paf, bool fAdd )
{
    ...
    case APPLY_MANA:          ch->base_mana		+= mod;	break;
    case APPLY_HIT:           ch->base_hit		+= mod;	break;
    case APPLY_MOVE:          ch->base_move		+= mod;	break;
     ...
     * Talent apply types
     */
    case APPLY_FIRE_MAGIC:	ch->mod_talent[TAL_FIRE] += mod; break;
    case APPLY_EARTH_MAGIC:	ch->mod_talent[TAL_EARTH] += mod; break;
    case APPLY_WIND_MAGIC:	ch->mod_talent[TAL_WIND] += mod; break;
    case APPLY_FROST_MAGIC:	ch->mod_talent[TAL_FROST] += mod; break;
    case APPLY_LIGHTNING_MAGIC:	ch->mod_talent[TAL_LIGHTNING] += mod; break;
    case APPLY_WATER_MAGIC:	ch->mod_talent[TAL_WATER] += mod; break;
    case APPLY_DREAM_MAGIC:	ch->mod_talent[TAL_DREAM] += mod; break;
    case APPLY_SPEECH_MAGIC:	ch->mod_talent[TAL_SPEECH] += mod; break;
    case APPLY_HEALING_MAGIC:	ch->mod_talent[TAL_HEALING] += mod; break;
    case APPLY_DEATH_MAGIC:	ch->mod_talent[TAL_DEATH] += mod; break;
    case APPLY_CHANGE_MAGIC:	ch->mod_talent[TAL_CHANGE] += mod; break;
    case APPLY_TIME_MAGIC:	ch->mod_talent[TAL_TIME] += mod; break;
    case APPLY_MOTION_MAGIC:	ch->mod_talent[TAL_MOTION] += mod; break;
    case APPLY_MIND_MAGIC:	ch->mod_talent[TAL_MIND] += mod; break;
    case APPLY_ILLUSION_MAGIC:	ch->mod_talent[TAL_ILLUSION] += mod; break;
    case APPLY_SEEKING_MAGIC:	ch->mod_talent[TAL_SEEKING] += mod; break;
    case APPLY_SECURITY_MAGIC:	ch->mod_talent[TAL_SECURITY] += mod; break;
    case APPLY_CATALYSM_MAGIC:	ch->mod_talent[TAL_CATALYSM] += mod; break;
    case APPLY_VOID_MAGIC:	ch->mod_talent[TAL_VOID] += mod; break;
...
/* spell cast upon wear/removal of an object	-Thoric */
	...
	&&   skill->type == SKILL_SPELL )
	{
	   ...
	   if ( (retcode=(*skill->spell_fun) ( mod, get_curr_wil(ch), ch,ch )) == rCHAR_DIED
	   ||   char_died(ch) )
	      return;
	}
	break;
...
void update_aris(CHAR_DATA *ch)
{
	...
	if (ch->singing) {
	    switch (ch->singing) {
		case SONG_FLIGHT:
		    xSET_BIT(ch->affected_by, AFF_FLYING); break;
		case SONG_SIGHT:
		    xSET_BIT(ch->affected_by, AFF_TRUESIGHT); break;
		case SONG_SHADOWS:
		    xSET_BIT(ch->affected_by, AFF_DARK); break;
		case SONG_LIGHT:
		    xSET_BIT(ch->affected_by, AFF_GLOW); break;
	    }
	}
...
void char_to_room( CHAR_DATA *ch, ROOM_INDEX_DATA *pRoomIndex )
{
    ...
    if ( !pRoomIndex )
    {
		if (char_died(ch)) { /* Ack! Don't want to put dead chars back in the game! -- Scion */
...
int apply_ac( OBJ_DATA *obj, int iWear )
{
    ...
    case WEAR_HAND:	return     obj->value[0];
...
char * pos_string( CHAR_DATA *victim )
{
	switch (victim->position)
	{
		...
		case POS_SITTING:	return "is sitting";
		case POS_KNEELING:	return "is kneeling";
	}
	return "ERROR";
}
...
void dump_container(OBJ_DATA *obj) {
	...
	for (obj1 = obj->first_content; obj1; obj1 = obj->first_content) {
		...
		if (obj->in_room) {
			...
			bug("Obj %d could not be removed from container, put in Void for disposal.", 
				obj1->pIndexData->vnum);
...
bool ms_find_obj( CHAR_DATA *ch )
{
	...
	switch( number_range( 1, sub/10 ) )
	{
	    ...
	    case  6: t="Which one?!?  You see two... no three...\n\r"; break;
	}
    }
    send_to_char( t, ch );
    return TRUE;
...
bool can_see( CHAR_DATA *ch, CHAR_DATA *victim )
{
    ...
    if (( !IS_NPC(victim)
    &&   xIS_SET(victim->act, PLR_WIZINVIS)
    &&  TALENT(ch, TAL_SEEKING) < victim->pcdata->wizinvis ))
	return FALSE;

    /* SB */
    if ( IS_NPC(victim)
    &&   xIS_SET(victim->act, ACT_MOBINVIS)
    &&  TALENT(ch, TAL_SEEKING) < victim->mobinvis )
        return FALSE;
    ...
    &&  victim->pcdata->noncombat[SK_STEALTH]*5
    >   get_curr_per(ch) + TALENT(ch, TAL_SEEKING))
	return FALSE;

    if ( IS_AFFECTED(victim, AFF_DREAMWORLD)
    && !IS_AFFECTED(ch, AFF_DREAMWORLD)
    && TALENT(ch, TAL_SEEKING) + TALENT(ch, TAL_DREAM) < 100 )
          return FALSE;

    if (IS_AFFECTED(victim, AFF_VOID)
    && !IS_AFFECTED(ch, AFF_VOID)
    && TALENT(ch, TAL_VOID) < 25)
	return FALSE;
...
char *affect_loc_name( int location )
{
    ...
    case APPLY_WIS:		return "willpower";
    case APPLY_CON:		return "constitution";
    case APPLY_CHA:		return "perception";
    case APPLY_FIRE_MAGIC:	return "fire magic";
    case APPLY_EARTH_MAGIC:	return "earth magic";
    case APPLY_WIND_MAGIC:	return "wind magic";
    case APPLY_FROST_MAGIC:	return "frost magic";
    case APPLY_LIGHTNING_MAGIC:	return "lightning magic";
    case APPLY_WATER_MAGIC:	return "water magic";
    case APPLY_DREAM_MAGIC:	return "dream magic";
    case APPLY_SPEECH_MAGIC:	return "speech magic";
    case APPLY_HEALING_MAGIC:	return "healing magic";
    case APPLY_DEATH_MAGIC:	return "death magic";
    case APPLY_CHANGE_MAGIC:	return "change magic";
    case APPLY_TIME_MAGIC:	return "time magic";
    case APPLY_MIND_MAGIC:	return "mind magic";
    case APPLY_MOTION_MAGIC:	return "motion magic";
    case APPLY_ILLUSION_MAGIC:	return "illusion magic";
    case APPLY_SEEKING_MAGIC:	return "seeking magic";
    case APPLY_SECURITY_MAGIC:	return "security magic";
    case APPLY_CATALYSM_MAGIC:	return "catalysm magic";
    case APPLY_VOID_MAGIC:	return "void magic";
    case APPLY_LCK:		return "endurance";
...
char *affect_bit_name( EXT_BV *vector )
{
    ...
    if ( xIS_SET(*vector, AFF_DETECT_EVIL  )) strcat( buf, " detect_align" );
    ...
    if ( xIS_SET(*vector, AFF_INFRARED     )) strcat( buf, " infravision" );
    ...
    if ( xIS_SET(*vector, AFF_FLOATING     )) strcat( buf, " feather_fall"  );
    if ( xIS_SET(*vector, AFF_TRUESIGHT    )) strcat( buf, " true_sight"    );
    if ( xIS_SET(*vector, AFF_DETECTTRAPS  )) strcat( buf, " detect_traps"  );
    if ( xIS_SET(*vector, AFF_ANTHRO      )) strcat( buf, " anthro" );
    if ( xIS_SET(*vector, AFF_BERSERK      )) strcat( buf, " berserk"       );
    if ( xIS_SET(*vector, AFF_AQUA_BREATH  )) strcat( buf, " breathing" );
	if ( xIS_SET(*vector, AFF_HASTE		   )) strcat( buf, " haste"			);
	if ( xIS_SET(*vector, AFF_SLOW		   )) strcat( buf, " slow"			);
    if ( xIS_SET(*vector, AFF_DEAF	)) strcat( buf, " deaf" );
    if ( xIS_SET(*vector, AFF_DARK		)) strcat( buf, " dark" );
    if ( xIS_SET(*vector, AFF_GLOW		)) strcat( buf, " glow" );
    if ( xIS_SET(*vector, AFF_AQUATIC		)) strcat(buf, " aquatic" );
    if ( xIS_SET(*vector, AFF_SILENT		)) strcat(buf, " silent");
    if ( xIS_SET(*vector, AFF_UNDEAD		)) strcat(buf, " undead");
    if ( xIS_SET(*vector, AFF_DEMON		)) strcat(buf, " demon");
    if ( xIS_SET(*vector, AFF_MOUNTABLE		)) strcat(buf, " mountable");
    if ( xIS_SET(*vector, AFF_MOUNTED		)) strcat(buf, " mounted");
    if ( xIS_SET(*vector, AFF_COLDBLOOD		)) strcat(buf, " coldblood");
    if ( xIS_SET(*vector, AFF_FELINE		)) strcat(buf, " feline");
    if ( xIS_SET(*vector, AFF_NUISANCE		)) strcat(buf, " hellcurse");
    if ( xIS_SET(*vector, AFF_NONLIVING		)) strcat(buf, " nonliving");
    if ( xIS_SET(*vector, AFF_CONSTRUCT		)) strcat(buf, " construct");
    if ( xIS_SET(*vector, AFF_ETHEREAL		)) strcat(buf, " ethereal");
    if ( xIS_SET(*vector, AFF_NO_CORPSE		)) strcat(buf, " no_corpse");
    if ( xIS_SET(*vector, AFF_UNHOLY		)) strcat(buf, " unholy");
    if ( xIS_SET(*vector, AFF_HOLY		)) strcat(buf, " holy");
    if ( xIS_SET(*vector, AFF_ANGEL		)) strcat(buf, " angel");
    if ( xIS_SET(*vector, AFF_BEAUTY		)) strcat(buf, " beauty");
    if ( xIS_SET(*vector, AFF_FEAR		)) strcat(buf, " fear");
    if ( xIS_SET(*vector, AFF_HAS_ARTI		)) strcat(buf, " has_artifact");
    if ( xIS_SET(*vector, AFF_MAGICAL		)) strcat(buf, " magical");
...
char *extra_bit_name( EXT_BV *extra_flags )
{
    ...
    if ( xIS_SET(*extra_flags, ITEM_HOVER)	) strcat( buf, " hover" );
    if ( xIS_SET(*extra_flags, ITEM_NO_RESET)) strcat( buf, " noreset" );
    ...
    if ( xIS_SET(*extra_flags, ITEM_DREAMWORLD)	) strcat( buf, " dreamworld"    );
    if ( xIS_SET(*extra_flags, ITEM_EXPLOSIVE)	) strcat( buf, " explosive"   );
    if ( xIS_SET(*extra_flags, ITEM_FLAMMABLE)) strcat( buf, " flammable");
    if ( xIS_SET(*extra_flags, ITEM_NO_TAKE)) strcat( buf, " no-take");
    if ( xIS_SET(*extra_flags, ITEM_TWO_HANDED)	) strcat( buf, " two-handed"   );
    if ( xIS_SET(*extra_flags, ITEM_SHIELD)) strcat( buf, " shield" );
    if ( xIS_SET(*extra_flags, ITEM_ORGANIC)	) strcat( buf, " organic"      );
    if ( xIS_SET(*extra_flags, ITEM_METAL)	) strcat( buf, " metal"        );
    if ( xIS_SET(*extra_flags, ITEM_RETURNING)   ) strcat( buf, " returning" );
    if ( xIS_SET(*extra_flags, ITEM_GEM)	) strcat( buf, " gem"         );
    if ( xIS_SET(*extra_flags, ITEM_ARTIFACT)   ) strcat( buf, " artifact" );
    if ( xIS_SET(*extra_flags, ITEM_PROTOTYPE)	) strcat( buf, " prototype"    );
    if ( xIS_SET(*extra_flags, ITEM_DURABLE)	) strcat( buf, " durable");
    return ( buf[0] != '\0' ) ? buf+1 : "none";
}
...
OBJ_DATA *group_object( OBJ_DATA *obj1, OBJ_DATA *obj2 )
{
	...
	for (paf1 = obj1->first_affect;paf1;paf1 = paf1->next) {
		if (!paf2 || !paf1) return obj2;
		if (paf1->location != paf2->location) return obj2;
		if (paf1->modifier != paf2->modifier) return obj2;
		paf2 = paf2->next;
	}
	}
...
RECIPE_DATA *find_recipe( int i ) {
        ...
        while (recipe) {
		if (recipe->number == i) return recipe;
                recipe=recipe->next;
        }
        return NULL;
}
...
INGRED_DATA *find_ingred( int i ) {
        ...
        while (ingred) {
                if (ingred->vnum == i) return ingred;
                ingred=ingred->next;
        }
        return NULL;
}
...
MAT_SHOP *find_matshop( int i ) {
        ...
        while (matshop) {
                if (matshop->room == i) return matshop;
		if (matshop == last_matshop) break;
