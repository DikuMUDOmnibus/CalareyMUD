...
/* Changed below from 1 to 0 */
  ...
  if ( !str_cmp(chck, "ovnumcarry") && !chkobj) /* with no var, var version is above */
  {
    OBJ_DATA *pObj;
    int vnum = atoi(cvar);
...
/* Changed below from 1 to 0 for == 0 Shaddai */
  ...
  if ( chkchar )
  {
    ...
    {
      return chkchar->last_hit ? TRUE : FALSE;
    }
    ...
    if ( !str_cmp(chck, "worth") )
    {
      return mprog_veval(get_char_worth(chkchar), opr, atoi(rval), mob);
    }
    ...
    {
      if ( !IS_NPC(chkchar) )
        return mprog_seval(chkchar->species, opr, rval, mob);
      return mprog_seval(npc_race[chkchar->race], opr, rval, mob);
    }
    if ( !str_cmp(chck, "inborn") )
    {
      if (IS_NPC(chkchar) )
	return FALSE;
      return mprog_veval(chkchar->pcdata->inborn, opr,atoi(rval), mob);
    }
  ...
  if ( chkobj )
  {
	...
	if ( !str_cmp(chck, "ovnumcarry") ) /* with var, no var version is below */
  {
    OBJ_DATA *pObj;
    int vnum = chkobj->pIndexData->vnum;
  ...
  if ( !str_cmp(chck, "mortinarea") )   /* -- Gorog */
  {
	 ...
	 if ((anchor = get_char_world(mob, cvar))==NULL)
		 anchor = mob;
	 ...
	 case 'v':
		 if (obj && !obj_extracted(obj)) {
			 if (can_see_obj(mob, obj))
				 sprintf(t, "%d", obj->pIndexData->vnum);
			 else
				 strcpy(t, "-1");
		 } else
			 strcpy(t, "-1");
	 ...
	 case 'V':
		 if (v_obj && !obj_extracted(v_obj)) {
			 if (can_see_obj(mob, v_obj))
				 sprintf(t, "%d", v_obj->pIndexData->vnum);
			 else
				 strcpy(t, "-1");
		 } else
			 strcpy(t, "-1");
...
/*  Gonna try without this and see how far I can trust the builders -- Scion
...
void progbug( char *str, CHAR_DATA *mob )
{
  ...
  if (!IS_NPC(mob)) {
	bug("Mob command used by player.", 0);
	return;
  }
