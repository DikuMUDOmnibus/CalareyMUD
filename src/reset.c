...
/*    if ( arg[0] == '\0' )
    ...
    if ( !(pRoom = find_room(ch, argument, aRoom)) )
      return;
*/    pReset = make_reset('M', 0, pMob->vnum, 0, pRoom->vnum);
...
void instaroom( AREA_DATA *pArea, ROOM_INDEX_DATA *pRoom, bool dodoors )
{
  ...
  {
    if ( !IS_NPC(rch) || rch->pIndexData->vnum == 17)
      continue;
  ...
  {
	if (!IS_OBJ_STAT(obj, ITEM_PLRBLD)
	&&  !IS_OBJ_STAT(obj, ITEM_ARTIFACT)
	&&  !IS_OBJ_STAT(obj, ITEM_NO_RESET))
...
void do_instazone( CHAR_DATA *ch, char *argument )
{
  ...
  for ( vnum = pArea->low_r_vnum; vnum <= pArea->hi_r_vnum; vnum++ )
  {
	...
	send_to_char("Resets on all areas reset.\n\r", ch);
	return;
	}

  if ( IS_NPC(ch) || !ch->pcdata || !ch->pcdata->area )
  {
    send_to_char( "You don't have an assigned area to create resets for.\n\r",
        ch );
    return;
...
void reset_area( AREA_DATA *pArea )
{
  ...
  if (physicalobjects < (top_area*MAX_OBJS))
	  loadObj = TRUE;
  
  if (nummobsloaded < (top_area*MAX_MOBS))
	  loadMob = TRUE;
  ...
  lastobj = NULL;
	...
	/* Generate some generic mobs for the zone */
  if (pArea->first_race && pArea->nmob < pArea->nplayer * 50) {
	GENERIC_MOB *genmob;

	for (genmob = pArea->first_race;genmob;genmob = genmob->next) {
		mob = generate_mob(genmob->nation);
		mob_to_area(mob, pArea);
	}
  }

  /* Update vegetation in each room -- Scion */
  for ( i = pArea->low_r_vnum; i <= pArea->hi_r_vnum; i++) {
	  if ((pRoomIndex = get_room_index(i)) == NULL)
		  continue;

	if (IS_SET(pRoomIndex->room_flags, ROOM_BURNING)) {
	  if (pRoomIndex->curr_vegetation > 0
	   && pRoomIndex->curr_water < 30) {
		pRoomIndex->curr_vegetation -= 25;
	  } else {
		REMOVE_BIT(pRoomIndex->room_flags, ROOM_BURNING);
	  }
	} else {
	  if (pRoomIndex->curr_vegetation < pRoomIndex->vegetation)
		  pRoomIndex->curr_vegetation++;
	}
  }
...
*/			continue;
		...
		/* Find existing mobs and match them up to inactive resets */
		if (pReset->active == FALSE) { 
			...
			for (ch = first_char; ch; ch = ch->next) {
				...
				if (ch->pIndexData->vnum != pReset->arg1)
					continue;
...
*/
      ...
      obj = create_object(pObjIndex, 0);
	if (pRoomIndex->pShop) xSET_BIT(obj->extra_flags, ITEM_INVENTORY);
  ...
  for ( pReset = pArea->first_reset; pReset; pReset = pReset->next )
  {
    ...
    case 'M':
      ...
      if ( !(room = get_room_index(pReset->arg3)) )
        ...
        rname = room->name;
      sprintf( pbuf, "%s (%d) -> %s (%d) [%s]", mname, pReset->arg1, rname,
          pReset->arg3, (pReset->active == TRUE ? "X" : " ") );
      if ( !room )
        mob = NULL;
